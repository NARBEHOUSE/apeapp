...
function MiniMacroBar({ label, current, target, showRemaining }: {
  label: string; current: number; target: number; showRemaining?: boolean;
}) {
  const pct = target > 0 ? Math.min((current / target) * 100, 100) : 0;
  const isOver = current > target && target > 0;
  const statusColor = macroStatusColor(current, target);
  const remaining = Math.max(0, Math.round(target) - Math.round(current));
  return (
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-[0.5625rem] font-medium text-text-muted">{label}</span>
        <span className="text-[0.5625rem] text-text-muted">
          {showRemaining
            ? <span className="font-medium" style={{ color: remaining === 0 ? statusColor : undefined }}>{remaining}g</span>
...
// Draggable entry
function DraggableEntry({ entry, emoji, onDelete, onToggleFavorite, onEditTime, onEdit, onCopy }: {
  entry: FoodEntry;
  emoji: string;
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 bg-surface-raised rounded-xl p-2 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[0.75rem] font-medium truncate">{entry.name}</div>
        <div className="flex items-center gap-1.5 text-[0.6875rem] text-text-muted mt-0.5">
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        ...
        <button onClick={() => onEditTime(entry)} className="text-[0.625rem] text-text-muted bg-surface rounded px-1 py-0.5 flex items-center gap-0.5">
        ...
        {confirming ? (
          <>
            <button onClick={() => setConfirming(false)} className="text-[0.5625rem] px-1 py-0.5 rounded bg-surface text-text-muted hover:text-text-primary transition-colors">
              Cancel
            </button>
            <button onClick={() => onDelete(entry.id)} className="text-[0.5625rem] px-1 py-0.5 rounded bg-danger text-white transition-colors">
              Delete
            </button>
          </>
        ) : (
        ...
        <div className="space-y-1 pt-0.5">
          {summary && (
            <div className="flex items-center gap-2.5 px-1 pb-1 border-b border-border/30">
              <span className="text-[0.625rem] font-semibold text-accent-orange">{summary.cals} cal</span>
              <span className="text-[0.625rem] text-text-muted">P{summary.protein}</span>
              <span className="text-[0.625rem] text-text-muted">C{summary.carbs}</span>
              <span className="text-[0.625rem] text-text-muted">F{summary.fat}</span>
              {summary.count > 1 && (
                <span className="text-[0.5625rem] text-text-muted/50">{summary.count} items</span>
              )}
              <button
...
export default function Nutrition({ profile, onUpdateProfile }: NutritionPageProps) {
  ...
  const customEmojiByName = useMemo(() => {
    ...
    for (const f of foodLibrary) if (f.emoji) map.set(f.name.toLowerCase(), f.emoji);
    return map;
  }, [foodLibrary]);
  const resolveEmoji = (name: string) => customEmojiByName.get(name.toLowerCase()) || getFoodEmoji(name);
  ...
  const [editEntryUnit, setEditEntryUnit] = useState<ServingUnit>('g');
  ...
  // The target that was actually active on the viewed day — not today's target —
  // so paging back through history shows the goal that applied then.
  const targets = getMacroTargetsForDate(profile, selectedDate);
  ...
  function handleEditEntry(entry: FoodEntry) {
    ...
    setEditEntryUnit(normaliseServingUnit(entry.servingUnit));
    setEditEntryServings(String(entry.servingsConsumed));
    setEditEntryIngredients(entry.ingredients?.map((i) => ({ ...i })) ?? []);
    setModal('edit-entry');
  }
  ...
  function handleQuickAdd(meal: SavedMeal) {
    addEntry({ date: selectedDate, name: meal.name, servingSize: meal.servingSize, servingUnit: meal.servingUnit, servingsConsumed: 1, calories: meal.calories, protein: meal.protein, carbs: meal.carbs, fat: meal.fat, fiber: meal.fiber, source: 'manual', mealType: 'snack', ingredients: meal.ingredients?.map((i) => ({ ...i })) });
    toast(`Added ${meal.name}`, 'success');
  }

  function handleQuickAddFavorite(entry: FoodEntry) {
    addEntry({ date: selectedDate, name: entry.name, brand: entry.brand, servingSize: entry.servingSize, servingUnit: entry.servingUnit, servingsConsumed: entry.servingsConsumed, calories: entry.calories, protein: entry.protein, carbs: entry.carbs, fat: entry.fat, fiber: entry.fiber, source: entry.source, fdcId: entry.fdcId, mealType: entry.mealType, ingredients: entry.ingredients?.map((i) => ({ ...i })) });
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          ...
          {isToday && <span className="text-[0.625rem] text-accent-orange">Today</span>}
        </div>
        <button type="button" onClick={() => changeDate(1)} className="p-2 rounded-lg hover:bg-surface-raised">
      ...
      <div className="flex justify-center gap-1.5">
        <button
          ...
          className={`text-[0.5rem] px-2 py-0.5 rounded-full font-semibold transition-colors ${!showRemaining ? 'bg-accent text-white' : 'text-text-muted hover:text-text-secondary'}`}
        >Eaten</button>
        <button
          type="button"
      ...
      <button
        onClick={() => {
          // Editing always edits your current/live target, regardless of which
          // past day you're viewing — seed from profile.macroTargets, not `targets`.
          setEditMacroProtein(String(profile.macroTargets.protein));
          setEditMacroCarbs(String(profile.macroTargets.carbs));
          setEditMacroFat(String(profile.macroTargets.fat));
          setEditMacroFiber(String(profile.fiberTarget ?? 30));
      ...
      >
        <div className="flex items-baseline gap-2 mb-2.5">
          {showRemaining
            ? <>
                <span className="text-2xl font-bold">{Math.max(0, Math.round(targets.calories) - Math.round(totals.calories))}</span>
                <span className="text-xs text-text-muted">kcal remaining</span>
              </>
            : <>
          ...
          <span className="text-[0.625rem] font-medium ml-auto px-1.5 py-0.5 rounded-full" style={{ backgroundColor: macroStatusBg(totals.calories, targets.calories), color: macroStatusColor(totals.calories, targets.calories) }}>
            {targets.calories > 0 ? Math.round((totals.calories / targets.calories) * 100) : 0}%
          </span>
        </div>
        <div className="flex gap-2">
          <MiniMacroBar label="Protein" current={totals.protein} target={targets.protein} showRemaining={showRemaining} />
          <MiniMacroBar label="Carbs"   current={totals.carbs}   target={targets.carbs}   showRemaining={showRemaining} />
          <MiniMacroBar label="Fat"     current={totals.fat}     target={targets.fat}     showRemaining={showRemaining} />
          <MiniMacroBar label="Fiber"   current={totals.fiber}   target={profile.fiberTarget ?? 30} showRemaining={showRemaining} />
        </div>
        <div className="text-[0.5625rem] text-text-muted text-center mt-1.5">Tap to edit targets</div>
      </button>
      ...
      {tab === 'planner' && (
        <>
              ...
              <div className="space-y-0">
                ...
                {!showAllHours && visibleStart > 0 && (
                  <button
                    ...
                    className="w-full text-[0.625rem] text-text-muted/50 hover:text-text-muted py-1 text-center"
                  >
          ...
          {entries.length > 0 && (
            <div className="text-center text-[0.625rem] text-text-muted">{entries.length} entries · {Math.round(totals.calories)} cal</div>
          )}
        </>
      )}
        ...
        <div className="space-y-4">
          ...
          <button onClick={() => { setEditingMeal(null); setAddAtTime(null); setModal('meal-builder'); }} className="w-full bg-accent-blue/10 border border-accent-blue/20 rounded-2xl p-4 flex items-center gap-3 active:scale-[0.98] transition-transform">
            ...
            <div>
              ...
              <div className="text-[0.6875rem] text-text-muted">Multi-ingredient meal with full macros</div>
            </div>
          </button>
                  ...
                  <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-0.5">
                    {favorites.filter((e) => !favSearch || e.name.toLowerCase().includes(favSearch.toLowerCase())).map((entry) => (
                      <div key={entry.id} className="bg-surface rounded-xl p-3 flex items-center gap-3">
                        <span className="text-lg">{resolveEmoji(entry.name)}</span>
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium truncate">{entry.name}</div>
                          <div className="text-[0.625rem] text-text-muted">{Math.round(entry.calories)} cal · P{Math.round(entry.protein)}g · C{Math.round(entry.carbs)}g · F{Math.round(entry.fat)}g</div>
                        </div>
                        ...
                        <button onClick={() => handleQuickAddFavorite(entry)} className="bg-surface-raised px-3 py-1.5 rounded-lg text-[0.625rem] font-medium text-accent-blue">+ Add</button>
                      </div>
                    ))}
                  </div>
                </>
          ...
          <div>
            ...
            {myFoodsOpen && (foodLibrary.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-[0.625rem] text-text-muted/70 mt-1">Use "Search & Save Food" or "Enter Manually" above to build your library</p>
              </div>
            ) : (
              <>
                <input
                ...
                {(() => {
                  ...
                  return (
                    <>
                      {!q && zeroCount > 0 && (
                        <div className="bg-warning/10 border border-warning/20 rounded-lg px-3 py-2 text-[0.625rem] text-warning mb-2">
                      ...
                      <div className="space-y-1 max-h-[300px] overflow-y-auto pr-0.5">
                            ...
                            return (
                              <div key={food.name} className="bg-surface rounded-xl p-3 space-y-2 border border-accent-blue/30">
                                  ...
                                  <div className="space-y-1 max-h-28 overflow-y-auto">
                                    {usdaFoodResults.map((r, ri) => (
                                      <button key={ri} onClick={() => {
                                        ...
                                        setEditFoodUnit('g');
                                        setUsdaFoodResults([]);
                                      }} className="w-full text-left bg-surface-raised rounded-md px-2 py-1 text-[0.625rem] hover:bg-border">
                                        <div className="flex items-center gap-1">
                                          <span className="font-medium truncate flex-1">{r.name}</span>
                                          <span className={`text-[0.5rem] px-1 rounded ${r.source === 'DB' ? 'bg-accent-blue/15 text-accent-blue' : 'bg-green-500/15 text-green-500'}`}>{r.source}</span>
                                        </div>
                                ...
                                <div className="grid grid-cols-5 gap-1">
                                  <div><label className="text-[0.5rem] text-text-muted">Cal</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFoodCal} onChange={(e) => setEditFoodCal(e.target.value)} /></div>
                                  <div><label className="text-[0.5rem] text-text-muted">Prot</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFoodP} onChange={(e) => setEditFoodP(e.target.value)} /></div>
                                  <div><label className="text-[0.5rem] text-text-muted">Carbs</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFoodC} onChange={(e) => setEditFoodC(e.target.value)} /></div>
                                  <div><label className="text-[0.5rem] text-text-muted">Fat</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFoodF} onChange={(e) => setEditFoodF(e.target.value)} /></div>
                                  <div><label className="text-[0.5rem] text-text-muted">Fiber</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFoodFiber} onChange={(e) => setEditFoodFiber(e.target.value)} /></div>
                                </div>
                                {/* Serving + Brand */}
                                <div className="grid grid-cols-2 gap-1">
                                  <div>
                                    <label className="text-[0.5rem] text-text-muted">Serving (g)</label>
                                  ...
                                  <div>
                                    <label className="text-[0.5rem] text-text-muted">Brand</label>
                                ...
                                {/* Barcode */}
                                <div>
                                  <label className="text-[0.5rem] text-text-muted">Barcode (optional)</label>
                          ...
                          return (
                            <div key={food.name} className="bg-surface-raised rounded-lg px-3 py-2 flex items-center gap-2">
                              ...
                              <button onClick={() => {
                                ...
                                {food.brand && <div className="text-[0.5625rem] text-text-muted truncate">{food.brand}</div>}
                                {hasMacros ? (
                                  <div className="text-[0.625rem] text-text-muted">
                                    ...
                                    {food.barcode && <span className="ml-1 text-[0.5rem] text-green-500">UPC</span>}
                                  </div>
                                ) : (
                                  <div className="text-[0.625rem] text-warning">Tap to add macros</div>
                                )}
                              </button>
                              {hasMacros && (
                                <button onClick={() => handleAddFromLibrary(food)} className="bg-surface rounded-lg px-2.5 py-1.5 text-[0.625rem] font-medium text-accent-blue shrink-0">+ Add</button>
                              )}
                              <button onClick={() => {
                        ...
                        {filtered.length > 100 && <p className="text-[0.625rem] text-text-muted text-center py-1">Showing 100 of {filtered.length} — search to narrow down</p>}
                      </div>
                    </>
                  );
                })()}
                      ...
                      <div key={meal.id} className="bg-surface rounded-xl p-3 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {meal.ingredients && meal.ingredients.length > 0 && (
                            <div className="text-[0.5625rem] text-text-muted/50 mt-0.5 truncate">{meal.ingredients.map((i) => i.name).join(', ')}</div>
                          )}
                        </div>
                        <button onClick={() => { setEditingMeal(meal); setAddAtTime(null); setModal('meal-builder'); }} className="px-2 py-1.5 rounded-lg text-[0.625rem] text-text-muted hover:text-accent-blue">Edit</button>
                        <button onClick={() => handleQuickAdd(meal)} className="bg-surface-raised px-3 py-1.5 rounded-lg text-[0.625rem] font-medium text-accent-blue">+ Add</button>
            ...
            {mealPlansOpen && entries.length > 0 && (
              ...
              >
                ...
                <div className="flex-1 text-left">
                  ...
                  <div className="text-[0.625rem] text-text-muted">{entries.length} items · {Math.round(totals.calories)} cal</div>
                </div>
              </button>
            )}
                  ...
                  <div key={plan.id} className="bg-surface rounded-xl p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="bg-accent-blue/10 text-accent-blue px-3 py-1.5 rounded-lg text-[0.625rem] font-semibold"
                      >
                        Apply
                      </button>
                      <button
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div>
              ...
              <div className="text-[0.6875rem] text-text-muted">Add ingredients, steps, and macros</div>
            </div>
          </button>
                  ...
                  <div key={recipe.id} className="bg-surface rounded-xl p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {recipe.tags.length > 0 && (
                          <div className="flex gap-1 mt-1 flex-wrap">
                            {recipe.tags.map((t) => (
                              <span key={t} className="text-[0.5625rem] bg-surface-raised px-1.5 py-0.5 rounded text-text-muted">{t}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex gap-2 mt-2 pt-2 border-t border-border">
                      <button
                        ...
                        className="flex-1 py-1.5 rounded-lg bg-accent-blue/10 text-accent-blue text-[0.625rem] font-semibold"
                      >
                        + Log 1 Serving
                      </button>
                      <button
                        onClick={() => { setEditingRecipe(recipe); setModal('recipe-editor'); }}
                        className="py-1.5 px-3 rounded-lg bg-surface-raised text-[0.625rem] text-text-muted font-medium"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => setDeleteRecipeId(recipe.id)}
                        className="py-1.5 px-3 rounded-lg bg-surface-raised text-[0.625rem] text-danger font-medium"
                      >
                        <Trash2 size={10} />
                      </button>
                    </div>
      ...
      {/* ===== CHARTS ===== */}
      {/* Charts always covers a fixed range ending today, independent of the Planner's selected date */}
      {tab === 'charts' && <NutritionCharts profileId={profile.id} targets={profile.macroTargets} fiberTarget={profile.fiberTarget ?? 30} />}
      ...
      <Modal open={modal === 'add'} onClose={() => { setModal(null); setAddAtTime(null); }} title="Add">
        <QuickAddSheet
          ...
          onMealSaved={() => setSavedMeals(getSavedMeals(profile.id))}
        />
      </Modal>
        ...
        <div className="space-y-4">
          {copyingEntry && (
            <div className="flex items-center gap-3 bg-surface rounded-xl p-3">
              <span className="text-lg">{resolveEmoji(copyingEntry.name)}</span>
      ...
      <Modal open={modal === 'edit-time'} onClose={() => { setModal(null); setEditingEntry(null); }} title="Change Time">
        <div className="space-y-4">
          {editingEntry && (
            <div className="flex items-center gap-3 bg-surface rounded-xl p-3">
              <span className="text-lg">{resolveEmoji(editingEntry.name)}</span>
      ...
      <Modal open={modal === 'edit-entry'} onClose={() => { setModal(null); setEditEntryData(null); }} title="Edit Food Entry">
        {editEntryData && editEntryIngredients.length > 0 && (() => {
          const totals = sumIngredients(editEntryIngredients);
          return (
          <div className="space-y-3">
            {/* Food name — locked */}
            <div className="flex items-center gap-3 bg-surface rounded-xl p-3">
              <span className="text-lg">{resolveEmoji(editEntryData.name)}</span>
              <div className="flex-1">
                <div className="text-sm font-medium">{editEntryData.name}</div>
                {editEntryData.brand && <div className="text-[0.625rem] text-text-muted">{editEntryData.brand}</div>}
              </div>
            </div>

            <IngredientEditor profileId={profile.id} ingredients={editEntryIngredients} onChange={setEditEntryIngredients} />

            <div className="bg-surface rounded-xl p-3 grid grid-cols-4 gap-2 text-center">
              <div><div className="text-lg font-bold text-accent-orange">{totals.cal}</div><div className="text-[0.5625rem] text-text-muted">kcal</div></div>
              <div><div className="text-lg font-bold text-accent-blue">{Math.round(totals.p)}g</div><div className="text-[0.5625rem] text-text-muted">protein</div></div>
              <div><div className="text-lg font-bold text-success">{Math.round(totals.c)}g</div><div className="text-[0.5625rem] text-text-muted">carbs</div></div>
              <div><div className="text-lg font-bold text-nutrition">{Math.round(totals.f)}g</div><div className="text-[0.5625rem] text-text-muted">fat</div></div>
            </div>

            <div className="text-[0.5625rem] text-accent-blue text-center">
              Only affects today's log — won't change the saved meal or other days
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                onClick={() => {
                  const totalGrams = editEntryIngredients.filter((i) => i.servingUnit === 'g').reduce((s, i) => s + i.amount, 0);
                  updateEntry(editEntryData.id, {
                    ingredients: editEntryIngredients,
                    calories: totals.cal,
                    protein: totals.p,
                    carbs: totals.c,
                    fat: totals.f,
                    fiber: totals.fiber ? totals.fiber : undefined,
                    servingSize: totalGrams || editEntryIngredients.reduce((s, i) => s + i.amount, 0),
                    servingUnit: totalGrams ? 'g' : (editEntryIngredients[0]?.servingUnit || 'serving'),
                    servingsConsumed: 1,
                  });
                ...
                disabled={editEntryIngredients.length === 0}
        ...
        {editEntryData && editEntryIngredients.length === 0 && (() => {
          ...
          const origUnit = normaliseServingUnit(editEntryData.servingUnit);
          ...
          // Convert to grams so g↔oz unit switches are accounted for correctly
          const origGrams = servingToGrams(origServing, origUnit);
          const newServing = parseFloat(editEntryServing) || origServing;
          const newGrams = servingToGrams(newServing, editEntryUnit);
          const newQty = parseFloat(editEntryServings) || 1;
          const gramFactor = origGrams > 0 ? newGrams / origGrams : 1;
          const scaleFactor = gramFactor * newQty;
          ...
          <div className="space-y-3">
            ...
            <div className="bg-surface-raised rounded-lg px-3 py-2 text-[0.625rem] text-text-muted">
              Base: {origServing}{origUnit} = {origCal} cal · P{origP}g · C{origC}g · F{origF}g
            </div>

            {/* Editable: serving size + unit + quantity */}
              ...
              <div>
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    value={editEntryServing}
                    onChange={(e) => setEditEntryServing(e.target.value)}
                  />
                  <select
                    className="input-field text-sm w-16 shrink-0"
                    value={editEntryUnit}
                    onChange={(e) => {
                      const newUnit = e.target.value as ServingUnit;
                      const converted = convertServingUnit(parseFloat(editEntryServing) || origServing, editEntryUnit, newUnit);
                      setEditEntryUnit(newUnit);
                      setEditEntryServing(String(converted));
                    }}
                  >
                    {SERVING_UNITS.map((u) => (
            ...
            <div className="bg-surface rounded-xl p-3 grid grid-cols-4 gap-2 text-center">
              <div><div className="text-lg font-bold text-accent-orange">{scaledCal}</div><div className="text-[0.5625rem] text-text-muted">kcal</div></div>
              <div><div className="text-lg font-bold text-accent-blue">{scaledP}g</div><div className="text-[0.5625rem] text-text-muted">protein</div></div>
              <div><div className="text-lg font-bold text-success">{scaledC}g</div><div className="text-[0.5625rem] text-text-muted">carbs</div></div>
              <div><div className="text-lg font-bold text-nutrition">{scaledF}g</div><div className="text-[0.5625rem] text-text-muted">fat</div></div>
            </div>

            <div className="text-[0.5625rem] text-accent-blue text-center">
              Macros scale with serving · unit converts automatically · Edit base food in My Foods library
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => {
                updateEntry(editEntryData.id, {
                  ...
                  servingUnit: editEntryUnit,
                  servingsConsumed: newQty,
                  calories: Math.round(origCal * gramFactor),
                  protein: Math.round(origP * gramFactor * 10) / 10,
                  carbs: Math.round(origC * gramFactor * 10) / 10,
                  fat: Math.round(origF * gramFactor * 10) / 10,
                  fiber: origFiber > 0 ? Math.round(origFiber * gramFactor * 10) / 10 : undefined,
                });
                setModal(null);
                setEditEntryData(null);
      ...
      <Modal open={modal === 'edit-macros'} onClose={() => setModal(null)} title="Edit Macro Targets">
        <div className="space-y-3">
          <div className="bg-surface rounded-xl p-3 text-center">
            ...
            <div className="text-[0.625rem] text-text-muted uppercase tracking-wider mt-0.5">calories (auto-calculated)</div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <div className="text-[0.5rem] text-text-muted text-center mt-0.5">4 cal/g</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-[0.5rem] text-text-muted text-center mt-0.5">9 cal/g</div>
            </div>
          </div>
          <div>
                ...
                if (onUpdateProfile) {
                  onUpdateProfile(profile.id, {
                    macroTargets: {
                      ...
                      protein: parseInt(editMacroProtein) || profile.macroTargets.protein,
                      carbs: parseInt(editMacroCarbs) || profile.macroTargets.carbs,
                      fat: parseInt(editMacroFat) || profile.macroTargets.fat,
                    },
          ...
          <div className="bg-bg w-full max-w-md max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl pb-20 sm:pb-5">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  <div key={s.label} className="bg-surface rounded-xl p-2 text-center">
                    ...
                    <div className="text-[0.5625rem] text-text-muted uppercase">{s.label}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-surface rounded-xl p-3 text-xs">
                <div className="text-[0.625rem] text-text-muted font-semibold uppercase mb-1">Per Serving</div>
