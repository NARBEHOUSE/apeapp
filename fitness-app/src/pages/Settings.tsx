...
import type { Profile, BodyStats, FitnessGoal, ActivityLevel, Gender, MacroTargetHistoryEntry } from '../types';
import { useGoogleAuth } from '../contexts/GoogleAuthContext';
import { hasCoachScope, requestCoachAccess, requireAccessToken } from '../utils/googleAuth';
...
interface Props {
  profile: Profile;
  onUpdateProfile: (id: string, updates: Partial<Profile>, options?: { macroTargetEffectiveDate?: string }) => void;
  onSetMacroTargetHistory: (id: string, history: MacroTargetHistoryEntry[]) => void;
  profiles: Profile[];
...
export function Settings({ profile, onUpdateProfile, onSetMacroTargetHistory, profiles, onDeleteProfile, onLogout }: Props) {
  ...
  // When a macro-target edit actually took effect — defaults to today, but can be
  // backdated (e.g. "I actually switched 3 weeks ago") so history stays accurate.
  const [editEffectiveDate, setEditEffectiveDate] = useState(() => new Date().toISOString().split('T')[0]);

  // Goal History editor — direct add/edit/delete of tracked calorie/macro phases
  const [editingHistoryEntry, setEditingHistoryEntry] = useState<MacroTargetHistoryEntry | 'new' | null>(null);
  const [historyFormDate, setHistoryFormDate] = useState('');
  const [historyFormCalories, setHistoryFormCalories] = useState('');
  const [historyFormProtein, setHistoryFormProtein] = useState('');
  const [historyFormCarbs, setHistoryFormCarbs] = useState('');
  const [historyFormFat, setHistoryFormFat] = useState('');
  const [historyFormGoal, setHistoryFormGoal] = useState<FitnessGoal | ''>('');

  const recalcHistoryCaloriesFromMacros = (protein: string, carbs: string, fat: string) => {
    const cal = Math.round((parseFloat(protein) || 0) * 4 + (parseFloat(carbs) || 0) * 4 + (parseFloat(fat) || 0) * 9);
    setHistoryFormCalories(String(cal));
  };

  function openEditHistoryEntry(entry: MacroTargetHistoryEntry) {
    setEditingHistoryEntry(entry);
    setHistoryFormDate(entry.date);
    setHistoryFormCalories(String(entry.macroTargets.calories));
    setHistoryFormProtein(String(entry.macroTargets.protein));
    setHistoryFormCarbs(String(entry.macroTargets.carbs));
    setHistoryFormFat(String(entry.macroTargets.fat));
    setHistoryFormGoal(entry.fitnessGoal || '');
  }

  function openAddHistoryEntry() {
    setEditingHistoryEntry('new');
    setHistoryFormDate(new Date().toISOString().split('T')[0]);
    setHistoryFormCalories(String(profile.macroTargets.calories));
    setHistoryFormProtein(String(profile.macroTargets.protein));
    setHistoryFormCarbs(String(profile.macroTargets.carbs));
    setHistoryFormFat(String(profile.macroTargets.fat));
    setHistoryFormGoal(profile.bodyStats?.fitnessGoal || '');
  }

  function saveHistoryEntry() {
    if (!historyFormDate) return;
    const newEntry: MacroTargetHistoryEntry = {
      date: historyFormDate,
      macroTargets: {
        calories: parseInt(historyFormCalories) || 0,
        protein: parseInt(historyFormProtein) || 0,
        carbs: parseInt(historyFormCarbs) || 0,
        fat: parseInt(historyFormFat) || 0,
      },
      fitnessGoal: historyFormGoal || undefined,
    };
    const existing = profile.macroTargetHistory || [];
    const withoutOldDate = editingHistoryEntry && editingHistoryEntry !== 'new'
      ? existing.filter((h) => h.date !== editingHistoryEntry.date)
      : existing;
    const withoutNewDate = withoutOldDate.filter((h) => h.date !== newEntry.date);
    onSetMacroTargetHistory(profile.id, [...withoutNewDate, newEntry]);
    setEditingHistoryEntry(null);
    toast('Goal history updated', 'success');
  }

  function deleteHistoryEntry(date: string) {
    const existing = profile.macroTargetHistory || [];
    if (existing.length <= 1) {
      toast('You need at least one goal period', 'error');
      return;
    }
    onSetMacroTargetHistory(profile.id, existing.filter((h) => h.date !== date));
    setEditingHistoryEntry(null);
    toast('Goal period removed', 'success');
  }

  const sortedHistory = [...(profile.macroTargetHistory || [])].sort((a, b) => a.date.localeCompare(b.date));
  ...
  const checkAutoAdjust = async () => {
    ...
    try {
      const [measurements, allFood] = await Promise.all([
      ...
      const latestWeighIn = [...weightEntries].sort((a, b) => b.date.localeCompare(a.date))[0];
      setLatestWeightLbs(latestWeighIn ? (latestWeighIn.unit === 'kg' ? kgToLbs(latestWeighIn.weight) : latestWeighIn.weight) : null);
      const trackedCalories = allFood.map((e) => ({
        date: e.date,
      ...
      // baseline-recovery tracking existed, so it starts picking up "back on track" the
      // same way a newly-started correction does, instead of never checking.
      const activeOverride = profile.temporaryCalorieOverride;
      if (activeOverride && activeOverride.baselineWeightLbs == null && latestWeighIn) {
        const latestLbs = latestWeighIn.unit === 'kg' ? kgToLbs(latestWeighIn.weight) : latestWeighIn.weight;
  ...
  const applyAutoAdjust = () => {
    ...
    // Rescale the existing macro targets to the new calorie total (protein grams held
    // constant, carbs/fat proportionally scaled) instead of recomputing macros from body
    // stats — that recompute path ignored the profile's actual current macros entirely.
    const adjustedMacros = rescaleMacrosToCalories(profile.macroTargets, autoAdjustResult.newCalories);
  ...
  const startCorrection = () => {
    if (!autoAdjustResult?.correctionSuggestion) return;
    const cs = autoAdjustResult.correctionSuggestion;
    ...
    onUpdateProfile(profile.id, {
      ...
      temporaryCalorieOverride: {
        ...
        baselineWeightLbs: latestWeightLbs != null ? computeBaselineWeightLbs(latestWeightLbs, autoAdjustResult) : undefined,
      },
      calorieAdjustments: [
        ...(profile.calorieAdjustments || []),
        {
          date: startDate,
          previousCalories: profile.macroTargets.calories,
          newCalories: cs.calories,
          reason: cs.reason,
          avgWeeklyChange: autoAdjustResult.avgWeeklyChange,
          kind: 'temporary-correction' as const,
        },
      ],
    });

    setEditCalories(String(correctedMacros.calories));
    setEditProtein(String(correctedMacros.protein));
    setEditCarbs(String(correctedMacros.carbs));
    setEditFat(String(correctedMacros.fat));
    toast(`${cs.days}-day correction started: ${cs.calories} cal/day`, 'success');
    setAutoAdjustResult({ ...autoAdjustResult, adherenceIssue: false });
  };

  const resumeFromCorrection = () => {
    const ov = profile.temporaryCalorieOverride;
    if (!ov) return;
    const recovered = ov.baselineWeightLbs != null && latestWeightLbs != null
      && checkBaselineRecovery(latestWeightLbs, ov.baselineWeightLbs).reached;
    onUpdateProfile(profile.id, {
      ...
      calorieAdjustments: [
        ...(profile.calorieAdjustments || []),
        {
          date: new Date().toISOString().split('T')[0],
          previousCalories: profile.macroTargets.calories,
          newCalories: ov.resumeMacroTargets.calories,
          reason: recovered ? 'Resumed normal goal — weight back near baseline' : 'Resumed normal goal after temporary correction',
          avgWeeklyChange: 0,
          kind: 'resume' as const,
        },
      ],
    });
    setEditCalories(String(ov.resumeMacroTargets.calories));
    setEditProtein(String(ov.resumeMacroTargets.protein));
    setEditCarbs(String(ov.resumeMacroTargets.carbs));
    setEditFat(String(ov.resumeMacroTargets.fat));
  ...
  // Font size
  const [activeFontSize, setActiveFontSizeState] = useState<FontSize>(getActiveFontSize());
  ...
  const recalcCaloriesFromMacros = (protein: string, carbs: string, fat: string) => {
    const cal = Math.round((parseFloat(protein) || 0) * 4 + (parseFloat(carbs) || 0) * 4 + (parseFloat(fat) || 0) * 9);
    setEditCalories(String(cal));
  };
  ...
  const handleSaveProfile = () => {
    ...
    }, { macroTargetEffectiveDate: editEffectiveDate });
  ...
  return (
    <div className="space-y-3 pb-24">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-2">
                  ...
                  {pendingInvites.map((invite) => (
                    <div key={invite.id} className="p-3 rounded-xl bg-surface-raised border border-accent-orange/40 space-y-3">
                      <div>
                        ...
                        <div className="text-[0.625rem] text-text-muted">{invite.coachEmail} wants to coach you</div>
                      </div>
              ...
              {pendingInvites.length === 0 && myCoachRels.length === 0 && (
                <p className="text-[0.6875rem] text-text-muted">No coaches yet. Ask your coach to send you an invite from their app.</p>
              )}
            </div>
                    ...
                    <div key={client.fileId} className="p-3 rounded-xl bg-surface-raised border border-border">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          {client.clientEmail && <div className="text-[0.625rem] text-text-muted">{client.clientEmail}</div>}
                        </div>
                        <div className="flex gap-1">
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg text-[0.625rem] font-medium bg-accent-blue/10 text-accent-blue"
                          >
                            View
                          </button>
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg text-[0.625rem] font-medium bg-success/10 text-success"
                          >
                            Backup
                          </button>
                          <button
                ...
                {blockList.map((email) => (
                  <div key={email} className="flex items-center justify-between p-2.5 rounded-xl bg-surface-raised border border-border">
                    ...
                    <button
                      ...
                      className="text-[0.625rem] font-medium text-accent-blue"
                    >
                      Unblock
                    </button>
                  </div>
      ...
      {/* Appearance */}
      <div className="card">
        <SectionHeader section="theme" icon={Palette} title="Appearance" />
        {expanded.has('theme') && (
          <div className="pt-2 space-y-5">
            {/* Theme */}
            <div>
              <p className="text-xs text-text-muted mb-2 font-medium uppercase tracking-wide">Theme</p>
              {(() => {
                const idx = THEMES.findIndex(t => t.id === activeTheme);
                const theme = THEMES[idx] ?? THEMES[0];
                const prev = () => handleThemeChange(THEMES[(idx - 1 + THEMES.length) % THEMES.length].id);
                const next = () => handleThemeChange(THEMES[(idx + 1) % THEMES.length].id);
                return (
                  <div className="flex items-center gap-2">
                    <button onClick={prev} className="p-3 rounded-xl bg-surface-raised text-text-secondary active:scale-95 transition-all">
                      <ChevronLeft size={18} />
                    </button>
                    <div className="flex-1 flex items-center justify-center gap-3 py-3 rounded-xl bg-surface-raised">
                      <div
                        className="w-4 h-4 rounded-full border border-white/20 flex-shrink-0"
                        style={{ backgroundColor: theme.swatch }}
                      />
                      <span className="text-sm font-semibold text-text-primary">{theme.name}</span>
                      <span className="text-xs text-text-muted">{idx + 1}/{THEMES.length}</span>
                    </div>
                    <button onClick={next} className="p-3 rounded-xl bg-surface-raised text-text-secondary active:scale-95 transition-all">
            ...
            {/* Text size */}
            <div>
              <p className="text-xs text-text-muted mb-2 font-medium uppercase tracking-wide">Text Size</p>
              <div className="flex gap-2">
                {([['normal', 'A', 'text-sm'], ['large', 'A', 'text-base'], ['xl', 'A', 'text-lg']] as const).map(([size, label, cls]) => (
                  <button
                    key={size}
                    onClick={() => handleFontSizeChange(size)}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl font-medium transition-all bg-surface-raised ${
                      activeFontSize === size ? 'ring-2 ring-accent-orange text-text-primary' : 'text-text-secondary'
                    }`}
                  >
                    <span className={cls}>{label}</span>
                    <span className="text-xs capitalize">{size === 'normal' ? 'Normal' : size === 'large' ? 'Large' : 'XL'}</span>
                    {activeFontSize === size && <Check size={12} />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {claudeKey.trim() && (() => {
                  ...
                  return (
                    <span className={`text-[0.625rem] font-semibold px-2 py-0.5 rounded-full ${colors[p] || colors.unknown}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                {claudeEnabled && (
                  <span className="text-[0.625rem] font-semibold text-success bg-success/10 px-2 py-0.5 rounded-full">
                    Active
                  </span>
                )}
              </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            {profile.tdee && (
              <div className="bg-surface-raised rounded-xl p-4 border border-border-light">
                ...
                {profile.bodyStats && (
                  <div className="mt-2 text-[0.625rem] text-text-muted">
            ...
            <div className="bg-surface-raised rounded-xl p-4 border border-border-light">
              ...
              {adjustLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  {autoAdjustResult.adherenceIssue && !profile.temporaryCalorieOverride && (
                    <div className="space-y-2 pt-1 border-t border-border-light">
                      <p className="text-[0.625rem] text-text-muted">
                        Your {profile.macroTargets.calories} cal/day goal looks fine — the trend is explained by not eating at it, not a bad goal.
                      </p>
                      {autoAdjustResult.correctionSuggestion && (
                        <button
                          onClick={startCorrection}
                          className="w-full bg-accent-blue/15 text-accent-blue font-semibold text-sm rounded-xl px-4 py-2.5 border border-accent-blue/30 active:scale-95 transition-transform"
                        >
                          Start {autoAdjustResult.correctionSuggestion.days}-day correction ({autoAdjustResult.correctionSuggestion.calories} cal)
                        </button>
                      )}
                    </div>
                  )}
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[0.625rem] text-text-muted hover:text-text-secondary"
                  >
            ...
            {/* Active/expired temporary correction */}
            {profile.temporaryCalorieOverride && (() => {
              const ov = profile.temporaryCalorieOverride;
              const expired = new Date().toISOString().split('T')[0] > ov.untilDate;
              ...
              return (
                <div className="bg-surface-raised rounded-xl p-4 border border-border-light space-y-2">
                  <div className="flex items-center gap-2">
                    <TrendingUp size={14} className="text-accent-blue" />
                  ...
                  <button
                    onClick={resumeFromCorrection}
                    className={
                      readyToResume
                        ? 'w-full bg-accent-blue/15 text-accent-blue font-semibold text-sm rounded-xl px-4 py-2.5 border border-accent-blue/30 active:scale-95 transition-transform'
                        : 'w-full text-xs text-text-muted hover:text-text-secondary py-1'
                    }
                  >
                ...
                <div className="space-y-1.5">
                  {[...profile.calorieAdjustments].reverse().slice(0, 5).map((adj, i) => (
                    <div key={i} className="bg-surface-raised rounded-lg px-3 py-2 text-xs border border-border-light">
                      ...
                      <p className="text-text-muted mt-0.5 text-[0.625rem]">{adj.reason}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Weekly bar graph of daily calories</div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Weight trend line graph</div>
              </div>
              <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-[0.6875rem] text-text-muted">Track a specific measurement</div>
                </div>
                <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-[0.6875rem] text-text-muted">Track a specific exercise</div>
                </div>
                <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Weekly progress ring (requires active program)</div>
              </div>
              <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-[0.6875rem] text-text-muted">Reminder to complete your check-in</div>
                </div>
                <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Monthly view of workouts, nutrition, and check-ins</div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Daily water tracking with quick-add buttons</div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Daily step count with weekly trend</div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <div className="text-[0.6875rem] text-text-muted">Data-driven suggestions using your Claude API key (~$0.02/use)</div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                <div className="text-sm font-medium">AI Voice Mode <span className="text-[0.5625rem] px-1.5 py-0.5 rounded bg-warning/15 text-warning font-semibold ml-1">BETA</span></div>
                <div className="text-[0.6875rem] text-text-muted">{claudeEnabled ? 'Hands-free voice commands — accuracy may vary' : 'Requires Claude API key'}</div>
              </div>
              <button
        ...
        {expanded.has('profile') && (
          <div className="space-y-4 pt-2">
              ...
              <div className="space-y-1.5">
                ...
                {profile.profilePhoto && (
                  <button
                    ...
                    className="text-[0.625rem] text-danger block"
                  >
                    Remove
                  </button>
                )}
            ...
            <div className="border-t border-border pt-4">
              ...
              {/* Gender */}
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Sex</label>
                <div className="flex gap-2">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Birthday{editAge > 0 ? ` (${editAge} yrs)` : ''}</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Weight ({editUnits === 'metric' ? 'kg' : 'lbs'})</label>
                  <input
                    type="number"
                    inputMode="decimal"
                    className="input-field text-sm py-2.5"
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Height (ft)</label>
                  <input
                    type="number"
                    inputMode="numeric"
                    className="input-field text-sm py-2.5"
                ...
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Height (in)</label>
                  <input
                    type="number"
                    inputMode="numeric"
                    className="input-field text-sm py-2.5"
              ...
              {/* Body Fat % */}
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Body Fat % (optional)</label>
                <input
                  type="number"
                  inputMode="decimal"
                  className="input-field text-sm py-2.5"
                ...
                />
                <p className="text-[0.5625rem] text-text-muted mt-0.5">
              ...
              {/* Activity Level */}
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Activity Level</label>
                <select
              ...
              {/* Fitness Goal */}
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Fitness Goal</label>
                <div className="flex gap-2">
              ...
              {/* Daily Step Goal */}
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Daily Step Goal</label>
                <input
                  type="number"
                  inputMode="numeric"
                  className="input-field text-sm py-2.5"
            ...
            <div className="border-t border-border pt-4">
              ...
              <div className="mb-3">
                <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">
                  Effective from
                </label>
                <input
                  type="date"
                ...
                />
                <p className="text-[0.625rem] text-text-muted mt-1">
                  Backdate this if the change already took effect earlier — history before this date keeps your old target.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="input-field text-sm py-2.5"
                    value={editProtein}
                    onChange={(e) => { setEditProtein(e.target.value); recalcCaloriesFromMacros(e.target.value, editCarbs, editFat); }}
                  />
                </div>
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">
                  ...
                  <input
                    ...
                    className="input-field text-sm py-2.5"
                    value={editCarbs}
                    onChange={(e) => { setEditCarbs(e.target.value); recalcCaloriesFromMacros(editProtein, e.target.value, editFat); }}
                  />
                </div>
                <div>
                  <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">
                  ...
                  <input
                    ...
                    className="input-field text-sm py-2.5"
                    value={editFat}
                    onChange={(e) => { setEditFat(e.target.value); recalcCaloriesFromMacros(editProtein, editCarbs, e.target.value); }}
                  />
                </div>
              </div>
              <div className="mt-3">
            ...
            <div className="border-t border-border pt-4">
              <div className="flex items-center justify-between mb-1.5">
                <label className="label">Goal History</label>
                <button onClick={openAddHistoryEntry} className="text-[0.625rem] text-accent-blue font-medium">
                  + Add Period
                </button>
              </div>
              <p className="text-[0.625rem] text-text-muted mb-2">
                Each period applies from its start date until the next one begins. Tap one to fix its date or numbers.
              </p>
              <div className="space-y-1.5">
                {sortedHistory.length === 0 && (
                  <p className="text-xs text-text-muted text-center py-2">No tracked history yet</p>
                )}
                {sortedHistory.map((entry, i) => {
                  const nextEntry = sortedHistory[i + 1];
                  const isOngoing = !nextEntry;
                  return (
                    <button
                      key={entry.date}
                      onClick={() => openEditHistoryEntry(entry)}
                      className="w-full text-left bg-surface-raised rounded-lg px-3 py-2 flex items-center justify-between gap-2"
                    >
                      <div className="min-w-0">
                        <div className="text-xs font-medium">
                          {formatShortDate(entry.date)} {isOngoing ? '– ongoing' : `– ${formatShortDate(nextEntry.date)}`}
                        </div>
                        <div className="text-[0.625rem] text-text-muted truncate">
                          {entry.macroTargets.calories} cal · P{entry.macroTargets.protein}g · C{entry.macroTargets.carbs}g · F{entry.macroTargets.fat}g
                          {entry.fitnessGoal && ` · ${GOAL_LABELS[entry.fitnessGoal]}`}
                        </div>
                      </div>
        ...
        {expanded.has('reports') && (
          <div className="space-y-4 pt-2">
            ...
            <div className="bg-surface-raised rounded-xl p-3">
              <div className="text-[0.625rem] font-medium text-text-secondary mb-1.5">Report includes:</div>
              <div className="grid grid-cols-2 gap-1 text-[0.625rem] text-text-muted">
        ...
        {expanded.has('data') && (
          <div className="space-y-4 pt-2">
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              <div className="text-[0.625rem] text-text-muted space-y-1">
              ...
              {importHistory.length > 0 && (
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5 text-[0.625rem] font-semibold text-text-secondary uppercase tracking-wider">
                    <History size={11} />
                    Import History
                  </div>
                  <div className="space-y-1">
                    {importHistory.map((entry) => (
                      <div key={entry.id} className="rounded-xl bg-surface-raised border border-border-light px-3 py-2 text-[0.6875rem]">
                          ...
                          {revertingId === entry.id ? (
                            <div className="flex gap-1 shrink-0">
                              <button
                                ...
                                className="px-2 py-1 rounded-lg bg-danger text-white text-[0.625rem] font-semibold"
                              >
                                Confirm
                              </button>
                              <button
                                onClick={() => setRevertingId(null)}
                                className="px-2 py-1 rounded-lg bg-surface border border-border text-[0.625rem]"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 rounded-lg bg-surface border border-border text-[0.625rem] text-text-secondary hover:text-danger hover:border-danger transition-colors shrink-0"
                            >
                              <RotateCcw size={10} />
                              Revert
                            </button>
                ...
                <div className="rounded-xl border border-border bg-surface-raised p-3 space-y-3">
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">From</label>
                      <input
                        type="date"
                        value={dataCleanFrom}
                    ...
                    <div className="flex-1">
                      <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">To</label>
                      <input
                        type="date"
                        value={dataCleanTo}
                  ...
                  <div className="space-y-1.5">
                    <div className="text-[0.625rem] text-text-muted font-semibold">Data types to delete:</div>
                  ...
                  {dataCleanPreview && (
                    <div className="rounded-lg bg-surface border border-border-light p-2.5 space-y-1">
                      <div className="text-[0.625rem] font-semibold text-text-secondary">Preview — entries that will be deleted:</div>
                      ...
                      {Object.values(dataCleanPreview).every((n) => n === 0) && (
                        <div className="text-[0.6875rem] text-text-muted">No entries found in this range.</div>
                      )}
                    </div>
                  )}
                  <div className="flex gap-2">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {p.id === profile.id && (
                      <span className="text-[0.625rem] text-accent-blue font-semibold ml-2">
                        Active
                      </span>
                    )}
                  </div>
        ...
        {expanded.has('about') && (
          <div className="pt-4 pb-2 space-y-4">
            ...
            <div className="text-center space-y-1.5">
              ...
              <p className="text-[0.6875rem] text-text-muted tracking-[0.15em] uppercase">
      ...
      {/* Goal History entry editor */}
      <Modal
        open={editingHistoryEntry !== null}
        onClose={() => setEditingHistoryEntry(null)}
        title={editingHistoryEntry === 'new' ? 'Add Goal Period' : 'Edit Goal Period'}
      >
        <div className="space-y-3">
          <div>
            <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Start date</label>
            <input
              type="date"
              className="input-field text-sm py-2.5"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Calories (kcal)</label>
              <input
                type="number"
                inputMode="decimal"
                className="input-field text-sm py-2.5"
                value={historyFormCalories}
                onChange={(e) => setHistoryFormCalories(e.target.value)}
              />
            </div>
            <div>
              <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Protein (g)</label>
              <input
                type="number"
                inputMode="decimal"
                className="input-field text-sm py-2.5"
                value={historyFormProtein}
                onChange={(e) => { setHistoryFormProtein(e.target.value); recalcHistoryCaloriesFromMacros(e.target.value, historyFormCarbs, historyFormFat); }}
              />
            </div>
            <div>
              <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Carbs (g)</label>
              <input
                type="number"
                inputMode="decimal"
                className="input-field text-sm py-2.5"
                value={historyFormCarbs}
                onChange={(e) => { setHistoryFormCarbs(e.target.value); recalcHistoryCaloriesFromMacros(historyFormProtein, e.target.value, historyFormFat); }}
              />
            </div>
            <div>
              <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Fat (g)</label>
              <input
                type="number"
                inputMode="decimal"
                className="input-field text-sm py-2.5"
                value={historyFormFat}
                onChange={(e) => { setHistoryFormFat(e.target.value); recalcHistoryCaloriesFromMacros(historyFormProtein, historyFormCarbs, e.target.value); }}
              />
            </div>
          </div>
          <div>
            <label className="text-[0.625rem] text-text-muted font-semibold block mb-1">Goal (optional label)</label>
            <div className="flex gap-2">
              {(['lose', 'maintain', 'build'] as FitnessGoal[]).map((g) => (
                <button
                  key={g}
                  type="button"
                  onClick={() => setHistoryFormGoal(g === historyFormGoal ? '' : g)}
                  className={`flex-1 py-1.5 text-xs rounded-lg font-medium transition-colors ${
                    historyFormGoal === g ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted'
                  }`}
                >
                  {GOAL_LABELS[g]}
                </button>
          ...
          <div className="flex gap-2 pt-1">
            {editingHistoryEntry !== 'new' && editingHistoryEntry && (
              <button
                onClick={() => deleteHistoryEntry(editingHistoryEntry.date)}
                className="p-2.5 rounded-xl bg-danger/10 text-danger"
                title="Delete this period"
              >
                <Trash2 size={16} />
              </button>
            )}
            <button onClick={() => setEditingHistoryEntry(null)} className="btn-secondary flex-1 text-sm">Cancel</button>
            <button onClick={saveHistoryEntry} className="btn-primary flex-1 text-sm">Save</button>
          </div>
        </div>
      </Modal>
