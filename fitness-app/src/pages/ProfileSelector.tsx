...
import { Plus, ChevronRight, ChevronLeft, Upload, Loader2, LogOut, Trash2 } from 'lucide-react';
            ...
            <div key={profile.id} className="bg-surface rounded-2xl overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <span className="text-[0.625rem] text-success font-medium px-2 py-0.5 rounded-full bg-success/10">Synced</span>
              ...
              <div className="flex border-t border-border">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[0.6875rem] text-text-muted hover:text-danger transition-colors"
                >
                  <LogOut size={12} />
                  Sign Out
                </button>
                  ...
                  <>
                    ...
                    {confirmDeleteId === profile.id ? (
                      <div className="flex-1 flex items-center justify-center gap-2 py-2.5">
                        <button
                          onClick={(e) => { e.stopPropagation(); onDelete(profile.id); setConfirmDeleteId(null); }}
                          className="text-[0.6875rem] text-danger font-medium"
                        >
                          Confirm Delete
                        </button>
                        <button
          ...
          {isSignedIn && profiles.length === 0 && (
            <div className="bg-surface rounded-2xl p-4 text-center space-y-3">
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="text-[0.625rem] text-success font-medium px-1.5 py-0.5 rounded-full bg-success/10">Synced</span>
              </div>
              ...
              <button
                ...
                className="text-[0.6875rem] text-text-muted hover:text-danger transition-colors"
              >
                Sign out of Google
              </button>
            </div>
            ...
            <div key={profile.id} className="bg-surface rounded-2xl overflow-hidden">
              {confirmDeleteId === profile.id ? (
                <div className="flex items-center gap-3 p-4">
                  <span className="flex-1 text-sm text-text-secondary">Delete <span className="font-medium text-text-primary">{profile.name}</span>? This won't delete workout or nutrition data.</span>
                  <button
                    onClick={() => { onDelete(profile.id); setConfirmDeleteId(null); }}
                    className="px-3 py-1.5 rounded-xl bg-danger text-white text-xs font-medium"
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => setConfirmDeleteId(null)}
                    className="px-3 py-1.5 rounded-xl bg-surface-2 text-text-muted text-xs"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <div className="flex items-center">
                  <button
                    onClick={() => onSelect(profile.id)}
                    className="flex-1 flex items-center gap-4 p-4 text-left active:scale-[0.98] transition-transform"
                  >
                    {profile.profilePhoto ? (
                      <img src={profile.profilePhoto} alt={profile.name} className="w-10 h-10 rounded-full object-cover shrink-0" />
                    ) : (
                  ...
                  {profiles.length > 1 && (
                    <button
                      ...
                      className="pr-4 pl-2 py-4 text-text-muted hover:text-danger transition-colors"
                    >
                      <Trash2 size={15} />
                    </button>
                  )}
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col px-6 pt-6 pb-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[0.6875rem] text-text-muted">{Math.min(stepIndex + 1, allSteps.length)}/{allSteps.length}</span>
      </div>
      ...
      {step === 'review' && macros && tdee && (
        <div className="flex-1 flex flex-col">
          ...
          <div className="text-center mb-8">
            ...
            <div className="text-text-muted text-[0.6875rem] mt-2">
              ...
              {[
                ...
                <div key={m.label} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[0.625rem] text-text-muted ml-2">{m.sub}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <p className="text-[0.6875rem] text-text-muted leading-relaxed px-1">
