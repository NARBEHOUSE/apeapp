    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50" onClick={onCancel}>
      <div className="w-full max-w-sm bg-surface rounded-t-2xl p-5 pb-8" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2">
          ...
          >
            <div className="text-[0.75rem] font-semibold text-warning">
              Update all {affectedCount} past {affectedCount === 1 ? 'entry' : 'entries'}
            </div>
            <div className="text-[0.6875rem] text-text-muted mt-0.5">Recalculates history based on new values</div>
          </button>
          ...
          >
            <div className="text-[0.75rem] font-semibold">Library only</div>
            <div className="text-[0.6875rem] text-text-muted mt-0.5">Updates food definition, keeps past entries unchanged</div>
          </button>
          ...
          <button
            ...
            className="w-full text-left bg-surface-raised border border-border rounded-xl px-4 py-3 active:scale-[0.98] transition-transform"
          >
            <div className="text-[0.75rem] font-semibold">Save as new food</div>
            <div className="text-[0.6875rem] text-text-muted mt-0.5">Creates a copy in your library, history stays intact</div>
          </button>
        </div>

        <button onClick={onCancel} className="w-full text-center text-[0.75rem] text-text-muted py-3 mt-2">
          Cancel
        </button>
      </div>
    </div>
