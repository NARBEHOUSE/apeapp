...
export function ImageCropper({ imageSrc, onCrop, onCancel, outputSize = 256 }: Props) {
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (touchesRef.current.size < 2) pinchStartDist.current = 0;
    if (touchesRef.current.size === 0) setDragging(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[300] flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3 mt-2">
        ...
        <span className="text-white/50 text-[0.625rem] tabular-nums w-10 text-center">{rotation.toFixed(1)}°</span>
      </div>
