  ...
  return (
    <div className="card">
      ...
      <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-0.5 bg-surface-raised rounded-md p-0.5 ml-1">
            {(['oz', 'ml'] as const).map((u) => (
              <button
                ...
                className={`px-1.5 py-0.5 rounded text-[0.625rem] font-semibold transition-colors ${waterUnit === u ? 'bg-accent-blue text-white' : 'text-text-muted'}`}
              >
                {u}
              </button>
            ))}
        ...
        <div className="flex items-center gap-2">
          {!expanded && <span className="text-[0.625rem] text-text-muted">Goal: {goal} {unitLabel}</span>}
          <button
      ...
      {expanded && (
        <div className="mb-3">
          ...
          <div className="flex gap-3 mb-3">
            ...
            <div className="flex-1 bg-surface-raised rounded-xl p-2.5 text-center">
              <p className="text-[0.625rem] text-text-muted mb-0.5">Goal hit</p>
          ...
          <SVGBarChart
            key={range}
            data={chartData.data.map((d) => ({ label: d.label, value: d.amount }))}
            color="#5b6ef5"
            targetValue={goal}
            targetLabel="Goal"
            height={140}
