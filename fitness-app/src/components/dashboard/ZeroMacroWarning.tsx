    ...
    return (
      <button onClick={startFixing} className="w-full bg-warning/10 border border-warning/20 rounded-2xl p-4 flex items-center gap-3 text-left active:scale-[0.98] transition-transform">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[0.6875rem] text-text-muted">Tap to review and update them one by one</div>
        </div>
  ...
  return (
    <div className="card border border-warning/20">
      <div className="flex items-center justify-between mb-2">
        <div className="text-[0.625rem] text-text-muted font-semibold">{currentIdx + 1} of {zeroFoods.length}</div>
      ...
      {autoMatched && (
        <div className="text-[0.625rem] text-green-500 font-medium mb-2 flex items-center gap-1">
      ...
      {!autoMatched && currentFood && (
        <button
          ...
          className="text-[0.625rem] text-accent-blue font-semibold mb-2 flex items-center gap-1 disabled:opacity-50"
        >
      ...
      {matches.length > 0 && (
        <div className="space-y-1 max-h-32 overflow-y-auto mb-2">
          {matches.map((m, i) => (
            <button key={i} onClick={() => selectMatch(m)} className="w-full text-left bg-surface-raised rounded-md px-2.5 py-1.5 text-[0.625rem] hover:bg-border transition-colors">
              <div className="flex items-center gap-1">
                <span className="font-medium truncate flex-1">{m.name}</span>
                <span className={`text-[0.5rem] px-1 rounded ${m.source === 'builtin' ? 'bg-accent-blue/15 text-accent-blue' : 'bg-green-500/15 text-green-500'}`}>
      ...
      <div className="grid grid-cols-5 gap-1 mb-2">
        <div><label className="text-[0.5rem] text-text-muted">Cal</label><input type="number" inputMode="decimal" className="input-field text-xs w-full py-1" value={editCal} onChange={(e) => setEditCal(e.target.value)} /></div>
        <div><label className="text-[0.5rem] text-text-muted">Prot</label><input type="number" inputMode="decimal" className="input-field text-xs w-full py-1" value={editP} onChange={(e) => setEditP(e.target.value)} /></div>
        <div><label className="text-[0.5rem] text-text-muted">Carbs</label><input type="number" inputMode="decimal" className="input-field text-xs w-full py-1" value={editC} onChange={(e) => setEditC(e.target.value)} /></div>
        <div><label className="text-[0.5rem] text-text-muted">Fat</label><input type="number" inputMode="decimal" className="input-field text-xs w-full py-1" value={editF} onChange={(e) => setEditF(e.target.value)} /></div>
        <div><label className="text-[0.5rem] text-text-muted">Fiber</label><input type="number" inputMode="decimal" className="input-field text-xs w-full py-1" value={editFiber} onChange={(e) => setEditFiber(e.target.value)} /></div>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={removeAndNext} className="py-2 px-3 rounded-lg bg-surface-raised text-[0.625rem] text-danger font-medium"><Trash2 size={10} className="inline mr-0.5" />Remove</button>
        <button onClick={skipFood} className="py-2 px-3 rounded-lg bg-surface-raised text-[0.625rem] text-text-muted font-medium flex-1">Skip</button>
        <button onClick={applyAndNext} disabled={!editCal && !editP} className="py-2 px-3 rounded-lg bg-accent-blue text-white text-[0.625rem] font-semibold flex-1 disabled:opacity-30 flex items-center justify-center gap-1">
