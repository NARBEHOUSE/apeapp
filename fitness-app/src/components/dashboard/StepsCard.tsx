...
export function StepsCard({ steps, profileId, stepGoal = DEFAULT_GOAL, onStepSaved }: Props) {
  ...
  useEffect(() => { setSelectedBars(new Set()); }, [expanded]);
  ...
  return (
    <div className="card">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {weekAvg > 0 && !expanded && (
            <span className="text-[0.625rem] text-text-muted">{weekAvg.toLocaleString()} avg/day</span>
          )}
          <button
      ...
      {!expanded && (() => {
        ...
        const selData = weekData.map((s, i) => ({ steps: s, i })).filter(({ i }) => selectedBars.has(i));
        const selTotal = selData.reduce((a, d) => a + d.steps, 0);
        ...
        return (
          <div className="mb-3">
            <div className="flex items-end gap-1" style={{ height: 40 }}>
              {weekData.map((s, i) => {
                ...
                return (
                  <button
                    key={i}
                    className="flex-1 flex flex-col items-center gap-0.5 h-full"
                  ...
                  >
                    <div className="flex-1 w-full flex items-end">
                      <div
                        ...
                        style={{
                          height: `${Math.max((s / maxSteps) * 100, s > 0 ? 10 : 5)}%`,
                          backgroundColor: '#5b6ef5',
                          opacity: isToday ? 1 : s > 0 ? 0.6 : 0.15,
                          outline: isSelected ? '1.5px solid rgba(255,255,255,0.5)' : 'none',
                          outlineOffset: '1px',
                        }}
                      />
                    </div>
                    <span className={`text-[0.5rem] font-medium ${isToday ? 'text-text-primary' : 'text-text-muted'}`}>
            ...
            {/* Selection stat row */}
            <div className="mt-1.5 min-h-[14px] text-[0.625rem]">
              {isAnySelected ? (
                <span className="text-text-muted">
                  {selData.length === 1 ? (
                    <><span className="font-semibold text-text-primary">{selTotal.toLocaleString()}</span>{' steps'}</>
                  ) : (
                    <><span className="font-semibold text-text-primary">{selAvg.toLocaleString()}</span>{' avg · '}<span className="text-text-muted">({selData.length}d)</span></>
                  )}
                </span>
              ) : null}
            </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex gap-3 mb-3">
            <div className="flex-1 bg-surface-raised rounded-xl p-2.5 text-center">
              <p className="text-[0.625rem] text-text-muted mb-0.5">{range}d avg</p>
              <p className="text-sm font-bold">{avgInRange > 0 ? avgInRange.toLocaleString() : '—'}</p>
            </div>
            <div className="flex-1 bg-surface-raised rounded-xl p-2.5 text-center">
              <p className="text-[0.625rem] text-text-muted mb-0.5">Days logged</p>
              <p className="text-sm font-bold">{daysLogged}/{range}</p>
            </div>
            <div className="flex-1 bg-surface-raised rounded-xl p-2.5 text-center">
              <p className="text-[0.625rem] text-text-muted mb-0.5">Goal</p>
          ...
          {editingDate && (
            <div className="mt-2 p-2.5 bg-surface-raised rounded-xl">
              <p className="text-[0.625rem] text-text-muted mb-1.5">
        ...
        >
          {todayEntry ? (
            <div>
              ...
              <span className="text-[0.625rem] text-text-muted ml-1">steps today</span>
            </div>
          ) : (
