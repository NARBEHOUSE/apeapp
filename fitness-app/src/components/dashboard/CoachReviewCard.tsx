  ...
  if (item.type === 'macros') {
    ...
    return (
      <div className="space-y-1">
          ...
          {(['protein', 'carbs', 'fat'] as const).map((key) => {
            ...
            return (
              <div key={key} className="text-center">
                <div className="text-[0.5625rem] text-text-muted uppercase">{key}</div>
                <div className="font-medium">{cur}g <span className="text-text-muted">→</span> {prop}g</div>
                {diff !== 0 && (
                  <div className={`text-[0.625rem] font-medium ${diff > 0 ? 'text-success' : 'text-danger'}`}>
                    {diff > 0 ? '+' : ''}{diff}g
                  </div>
                )}
              </div>
        ...
        <div className="text-center text-[0.625rem] text-text-muted">
  ...
  if (item.type === 'note') {
    try {
      ...
      if (parsed.action === 'set_questions' && Array.isArray(parsed.questions)) {
        return (
          <div className="space-y-1">
            <div className="text-[0.625rem] text-text-muted">{parsed.questions.length} questions:</div>
            ...
            <div key={item.id} className={`rounded-xl p-3 border transition-colors ${
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.coachNote && (
                    <div className="text-[0.625rem] text-text-muted mt-0.5">Coach: "{item.coachNote}"</div>
                  )}
                </div>
                {decision ? (
                  <div className="flex items-center gap-1">
                    <span className={`text-[0.625rem] font-medium ${decision.action === 'accepted' ? 'text-success' : 'text-danger'}`}>
                      {decision.action === 'accepted' ? 'Accepted' : 'Denied'}
                    </span>
                    <button onClick={() => { setDecisions((prev) => { const next = { ...prev }; delete next[item.id]; return next; }); }} className="text-[0.5625rem] text-text-muted underline ml-1">
                      Undo
                    </button>
                  </div>
                ) : (
              ...
              {isDenying && (
                <div className="mt-2 space-y-1.5">
                  ...
                  <div className="flex gap-1.5">
                    <button onClick={() => handleDenyWithoutNote(item.id)} className="text-[0.625rem] text-text-muted">
                      Deny without note
                    </button>
                    {denyNote.trim() && (
                      <button onClick={() => handleDeny(item.id)} className="text-[0.625rem] text-danger font-medium">
                        Deny with note
                      </button>
                    )}
                  </div>
              ...
              {decision?.clientNote && (
                <div className="mt-1 text-[0.625rem] text-text-muted italic">Your note: "{decision.clientNote}"</div>
              )}
            </div>
          );
        })}
