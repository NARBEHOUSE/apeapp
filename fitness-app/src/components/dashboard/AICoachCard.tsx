...
import { rescaleMacrosToCalories } from '../../utils/tdee';
...
export function AICoachCard({ profile, sessions, allFoodEntries, measurements, checkIns, steps, programs, onUpdateProfile }: Props) {
  ...
  const handleApply = useCallback((suggestion: CoachSuggestion) => {
    ...
    if (action.type === 'adjust_calories' && action.value) {
      // Changing the calorie total on its own would leave protein/carbs/fat targets
      // still summing to the old total — rescale them so the day's macro targets stay
      // consistent with the new calorie goal instead of going stale.
      targets = rescaleMacrosToCalories(profile.macroTargets, action.value);
    } else if (action.type === 'adjust_protein' && action.value) {
      targets.protein = action.value;
      targets.calories = targets.protein * 4 + targets.carbs * 4 + targets.fat * 9;
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {pendingCount > 0 && !expanded && (
              <span className="text-[0.625rem] bg-accent-blue/15 text-accent-blue font-semibold px-1.5 py-0.5 rounded-full">
                {pendingCount}
              </span>
            )}
          </div>
          {loading ? (
            ...
            <span className="text-[0.625rem] text-accent-blue font-semibold">Get Suggestions</span>
          )}
        </button>

        {/* Summary line when collapsed */}
        {response && !expanded && activeSuggestions.length > 0 && (
          <p className="text-[0.6875rem] text-text-muted mt-2">{response.summary}</p>
        )}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[0.5625rem] font-semibold uppercase ${colors.text}`}>{s.category}</span>
                        {isApplied && <Check size={10} className="text-success" />}
                      </div>
                      <div className="text-sm font-medium mt-0.5">{s.title}</div>
                      <p className="text-[0.6875rem] text-text-muted mt-1 leading-relaxed">{s.explanation}</p>
                    </div>
                    <button
