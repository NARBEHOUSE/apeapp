...
const SCHEME_OPTIONS: { value: SetSchemeType; label: string; desc: string }[] = [
  ...
  { value: 'last_set_amrap', label: 'Last Set AMRAP', desc: 'Working sets at target reps, last set is as many reps as possible' },
];
...
export function SetSchemeEditor({ scheme, sets, reps, onChange }: Props) {
  ...
  const handleTypeChange = (type: SetSchemeType) => {
    ...
    } else if (type === 'last_set_amrap') {
      onChange({ type, amrapWorkingSets: Math.max(1, sets - 1) });
    }
  };
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="text-[0.625rem] text-accent-blue font-semibold"
      >
      ...
      {expanded && (
        <div className="space-y-2">
          <div className="flex flex-wrap gap-1">
            {SCHEME_OPTIONS.map((opt) => (
              <button
                ...
                className={`px-2 py-1 rounded-lg text-[0.5625rem] font-semibold transition-colors ${
          ...
          {scheme?.type === 'top_set_backoff' && (
            <div className="bg-surface-raised rounded-lg p-2.5 space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[0.5rem] text-text-muted uppercase font-semibold">Top Set Reps</label>
                  <input
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="text-[0.5rem] text-text-muted uppercase font-semibold">Backoff Reps</label>
                  <input
                    type="text" inputMode="numeric" className="input-field text-xs py-1"
          ...
          {(scheme?.type === 'pyramid' || scheme?.type === 'reverse_pyramid') && scheme.pyramidReps && (
            <div className="bg-surface-raised rounded-lg p-2.5 space-y-2">
              <label className="text-[0.5rem] text-text-muted uppercase font-semibold">
              ...
              <div className="flex gap-1 flex-wrap">
                ...
                <button
                  ...
                  className="text-[0.5625rem] text-accent-blue font-semibold px-1"
                >+</button>
                {scheme.pyramidReps.length > 2 && (
                  <button
                    type="button"
                    onClick={() => onChange({ ...scheme, pyramidReps: scheme.pyramidReps!.slice(0, -1) })}
                    className="text-[0.5625rem] text-text-muted font-semibold px-1"
                  >−</button>
                )}
              </div>
              <p className="text-[0.5rem] text-text-muted">
          ...
          {scheme?.type === 'to_failure' && (
            <div className="bg-surface-raised rounded-lg p-2.5 space-y-1">
              ...
              <p className="text-[0.5rem] text-text-muted">
                Last {scheme.failureSets ?? sets} set{(scheme.failureSets ?? sets) > 1 ? 's' : ''} go to failure — preceding sets use target reps
              </p>
            </div>
          )}

          {/* Last Set AMRAP config */}
          {scheme?.type === 'last_set_amrap' && (
            <div className="bg-surface-raised rounded-lg p-2.5 space-y-1">
              <div className="flex items-center gap-2">
              ...
              <p className="text-[0.5rem] text-text-muted">
                {scheme.amrapWorkingSets ?? Math.max(1, sets - 1)} set{(scheme.amrapWorkingSets ?? Math.max(1, sets - 1)) !== 1 ? 's' : ''} at target reps, then 1 all-out AMRAP set
              </p>
            </div>
          )}
        </div>
