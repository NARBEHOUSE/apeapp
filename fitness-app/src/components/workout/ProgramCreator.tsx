              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {days.map((d) => (
                    ...
                    >
                      <div
                        className="w-6 h-6 rounded-md flex items-center justify-center text-white text-[0.625rem] font-bold flex-shrink-0"
                      ...
                      {preset.blocks.length > 0 && (
                        <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                          {preset.blocks.map((b, i) => (
                            <span
                              key={i}
                              className="text-[0.625rem] bg-surface-raised px-2 py-0.5 rounded-md text-text-secondary"
                            >
                              {b.name} ({b.weeks}w)
                            </span>
                          ))}
              ...
              <div>
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-1">
                  {blocks.map((block, i) => (
                    ...
                    >
                      <span className="text-[0.625rem] font-bold text-text-muted block mb-1">
                        BLOCK {i + 1}
                      </span>
                ...
                {days.map((day) => (
                  <div key={day.id} className="card">
                    <div className="flex items-center gap-3 mb-2">
                      <div
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-[0.625rem] font-bold flex-shrink-0"
                        style={{ backgroundColor: day.accent }}
                      >
                        {day.label.slice(0, 2)}
                      </div>
                          ...
                          <div key={ex.id}>
                            ...
                            {ex.progression && (
                              <p className="text-[0.625rem] text-text-muted">
                                {formatProgressionLabel(ex.progression as ExerciseProgression)}
                              </p>
                            )}
                          </div>
                        ))}
