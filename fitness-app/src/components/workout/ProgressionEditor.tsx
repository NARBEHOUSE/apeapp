    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-[0.625rem] text-text-muted hover:text-danger"
        >
          Remove
        </button>
      </div>
      ...
      <div className="bg-surface-raised rounded-lg p-2.5 space-y-2">
        {/* Type selector */}
        <div>
          <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Type</label>
          <select
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {!isCustom && (
            <button onClick={autoGenerate} className="text-[0.625rem] text-text-muted hover:text-text-secondary">
              Reset defaults
            </button>
          )}
          {!isCustom && calculatedTargets.length > 0 && !storedTargets && (
            <button onClick={customizeWeeks} className="text-[0.625rem] text-accent-blue hover:underline">
              Customize weeks
            </button>
          )}
          {storedTargets && !isCustom && (
            <button onClick={recalculate} className="text-[0.625rem] text-accent-blue hover:underline flex items-center gap-0.5">
        ...
        {hasTargets && (
          <div>
            <button
              ...
              className="text-[0.625rem] text-text-secondary font-medium flex items-center gap-1"
            >
            ...
            {showWeeks && (
              <div className="mt-1.5 space-y-1">
                ...
                <div className="grid grid-cols-[2rem_1fr_1fr_1fr_2rem] gap-1 px-0.5">
                  <span className="text-[0.5rem] text-text-muted text-center">Wk</span>
                  <span className="text-[0.5rem] text-text-muted">Sets</span>
                  <span className="text-[0.5rem] text-text-muted">Reps</span>
                  <span className="text-[0.5rem] text-text-muted">Weight</span>
                  <span className="text-[0.5rem] text-text-muted text-center">DL</span>
                </div>
                ...
                <div className="max-h-56 overflow-y-auto space-y-0.5">
                  {displayTargets.map((t, i) => (
                    ...
                    >
                      <span className={`text-[0.625rem] text-center font-mono ${t.isDeload ? 'text-[#f5a623]' : 'text-text-muted'}`}>
                        {t.week}
                      </span>
                      {isEditable ? (
                        <>
                        ...
                        <>
                          <span className="text-[0.625rem] text-text-secondary text-center">{t.sets}</span>
                          <span className="text-[0.625rem] text-text-secondary text-center">{t.reps}</span>
                          <span className="text-[0.625rem] text-text-secondary text-center">{t.weight}</span>
                          <span className="text-[0.625rem] text-center">{t.isDeload ? '↓' : ''}</span>
                        </>
                      )}
                    </div>
                  ))}
                ...
                {isEditable && (
                  <div className="flex gap-2 pt-1">
                    <button
                      onClick={addWeek}
                      className="text-[0.625rem] text-text-muted hover:text-text-secondary px-2 py-0.5 rounded bg-surface"
                    >
                      + Week
                    </button>
                    {displayTargets.length > 1 && (
                      <button
                        onClick={removeWeek}
                        className="text-[0.625rem] text-text-muted hover:text-danger px-2 py-0.5 rounded bg-surface"
                      >
                        - Week
                      </button>
                    )}
        ...
        {!hasTargets && exercise.startingWeight == null && !isCustom && (
          <p className="text-[0.5625rem] text-text-muted italic">
