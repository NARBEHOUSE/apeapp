...
const SET_TYPE_LABELS: Record<SetType, string> = { standard: '', warmup: 'W', dropset: 'DS', myoreps: 'MR', failure: 'F' };
const SET_TYPE_CLASSES: Record<SetType, string> = {
  standard: 'bg-surface-raised text-text-muted',
  warmup: 'bg-warning/20 text-warning',
  dropset: 'bg-accent-orange/20 text-accent-orange',
  myoreps: 'bg-accent-blue/20 text-accent-blue',
...
function ExerciseCard({
  ...
  onComplete: (exerciseId: string, weight: number, reps: number, rir?: number, rpe?: number, isWarmup?: boolean, setDuration?: number, setType?: SetType) => void;
  onUpdate: (exerciseId: string, setIndex: number, updates: Partial<SetLog>) => void;
  onSwap?: (swap: LibraryExercise) => void;
  onRename?: (exerciseId: string, newName: string) => void;
  onAddAlternative?: (exerciseId: string, altName: string) => void;
  ...
  const [setCount, setSetCount] = useState(() => {
    ...
    if (s?.type === 'to_failure') return exercise.sets;
    if (s?.type === 'last_set_amrap') return (s.amrapWorkingSets ?? Math.max(1, exercise.sets - 1)) + 1;
  ...
  const [renameValue, setRenameValue] = useState(exercise.name);
  const [customAltInput, setCustomAltInput] = useState('');
  const [customAltSuggestions, setCustomAltSuggestions] = useState<LibraryExercise[]>([]);
  ...
  const [inputs, setInputs] = useState<SetInput[]>(() => {
    ...
    const totalSets = scheme?.type === 'top_set_backoff' ? 1 + (scheme.backoffSets || 2)
      ...
      : scheme?.type === 'to_failure' ? exercise.sets
      : scheme?.type === 'last_set_amrap' ? (scheme.amrapWorkingSets ?? Math.max(1, exercise.sets - 1)) + 1
      : exercise.sets;

    const failureStartIdx = scheme?.type === 'to_failure'
      ? exercise.sets - (scheme.failureSets ?? exercise.sets)
      : -1;
    const amrapIdx = scheme?.type === 'last_set_amrap'
      ? (scheme.amrapWorkingSets ?? Math.max(1, exercise.sets - 1))
      : -1;
    ...
    return Array.from({ length: totalSets }, (_, i) => {
      ...
      } else if (scheme?.type === 'to_failure' && i >= failureStartIdx) {
        repStr = '';
      } else if (scheme?.type === 'last_set_amrap' && i >= amrapIdx) {
        repStr = '';
      } else {
        repStr = targetReps != null ? String(targetReps) : (last?.reps != null ? String(last.reps) : (prev?.reps != null ? String(prev.reps) : ''));
      }

      if (!repStr && exercise.reps && !(scheme?.type === 'to_failure' && i >= failureStartIdx) && !(scheme?.type === 'last_set_amrap' && i >= amrapIdx)) {
      ...
      let defaultSetType: SetType = 'standard';
      if (scheme?.type === 'to_failure' && i >= failureStartIdx) defaultSetType = 'failure';
      else if (scheme?.type === 'last_set_amrap' && i >= amrapIdx) defaultSetType = 'failure';
  ...
  const handleComplete = (setIndex: number) => {
    ...
    onComplete(exercise.id, weightInLbs, reps, rir, rpe, isWarmup, duration, input.setType);
  ...
  const addSet = () => {
    ...
    setInputs((prev) => [...prev, { weight: lastInput?.weight || '', reps: lastInput?.reps || '', duration: lastInput?.duration || '', effort: '', setType: 'standard' }]);
  };
  ...
  // Warmup sets go at the top of the list — right after any sets already completed,
  // so completed sets keep their sessionSets index alignment.
  const addWarmupSet = () => {
    const insertAt = sessionSets.filter((s) => s.completed).length;
    setSetCount((c) => c + 1);
    setInputs((prev) => {
      const next = [...prev];
      next.splice(insertAt, 0, { weight: '', reps: '', duration: '', effort: '', setType: 'warmup' });
      return next;
    });
  };
  ...
  const removeWarmupSet = () => {
    ...
    for (let i = inputs.length - 1; i >= 0; i--) {
      if (inputs[i]?.setType === 'warmup' && !sessionSets[i]?.completed) { idx = i; break; }
    }
    if (idx === -1) return;
    setSetCount((c) => c - 1);
  ...
  const hasRemovableWarmup = inputs.some((inp, i) => inp.setType === 'warmup' && !sessionSets[i]?.completed);
    ...
    <div className="bg-surface rounded-2xl p-4">
        ...
        {(onSkip || onRemove) && (
          <div className="flex items-center gap-1 shrink-0">
            {onSkip && (
              <button
                onClick={onSkip}
                className="p-1.5 rounded-lg hover:bg-surface-raised text-text-muted/50 hover:text-text-muted"
                title="Skip exercise"
              >
                <SkipForward size={13} />
              </button>
            ...
            {onRemove && (
              <button
                onClick={onRemove}
                className="p-1.5 rounded-lg hover:bg-surface-raised text-text-muted/50 hover:text-danger"
      ...
      {!collapsed && (
        <div className="mt-3 space-y-1.5">
          {/* Weekly target */}
          {weeklyTarget && (
            <div className={`flex items-center gap-2 px-1.5 py-1 rounded-lg text-[0.625rem] ${
          ...
          {exercise.inputType === 'time' ? (
            <div className="grid grid-cols-[1.5rem_1fr_1fr_2rem_2.25rem] gap-1.5 px-0.5">
              <span className="text-[0.5625rem] text-text-muted text-center">Type</span>
              <span className="text-[0.5625rem] text-text-muted">Weight (opt)</span>
              <span className="text-[0.5625rem] text-text-muted">Sec</span>
              <span />
              <span />
            </div>
          ) : (
            <div className={`grid ${effortMetric !== 'none' ? 'grid-cols-[1.5rem_1fr_1fr_2.5rem_2.25rem]' : 'grid-cols-[1.5rem_1fr_1fr_2.25rem]'} gap-1.5 px-0.5`}>
              <span className="text-[0.5625rem] text-text-muted text-center">Type</span>
              <span className="text-[0.5625rem] text-text-muted">Weight ({weightUnit})</span>
              <span className="text-[0.5625rem] text-text-muted">Reps</span>
              {effortMetric !== 'none' && <span className="text-[0.5625rem] text-accent-blue text-center">{effortMetric === 'rir' ? 'RIR' : 'RPE'}</span>}
              <span />
            </div>
          )}
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() => {
                    const cur = inputs[setIndex]?.setType || 'standard';
                    const next = SET_TYPE_CYCLE[(SET_TYPE_CYCLE.indexOf(cur) + 1) % SET_TYPE_CYCLE.length];
                    const updated = [...inputs];
                    updated[setIndex] = { ...updated[setIndex], setType: next };
                    setInputs(updated);
                  }}
                  className={`text-[0.625rem] font-bold text-center w-6 h-6 rounded flex items-center justify-center transition-colors ${
                    SET_TYPE_CLASSES[inputs[setIndex]?.setType || 'standard']
                  }`}
                  title="Tap to cycle set type"
                >
                  {inputs[setIndex]?.setType && inputs[setIndex].setType !== 'standard' ? (
                    <span className="text-[0.5rem]">{SET_TYPE_LABELS[inputs[setIndex].setType]}</span>
                  ) : scheme?.type === 'top_set_backoff' && setIndex === 0 ? (
                    <span className="text-[7px] text-accent font-bold">TOP</span>
                  ) : scheme?.type === 'last_set_amrap' && setIndex === setCount - 1 ? (
                    <span className="text-[7px] text-danger font-bold">AMR</span>
                  ) : (
                    <span className="text-[0.5625rem]">{setIndex + 1}</span>
                  )}
                </button>
                <input
                  type="number"
                ...
                {exercise.inputType === 'time' ? (
                  ...
                  <>
                    <input
                      ...
                      placeholder={
                        (scheme?.type === 'to_failure' && inputs[setIndex]?.setType === 'failure') ||
                        (scheme?.type === 'last_set_amrap' && setIndex === setCount - 1)
                          ? 'AMRAP'
                    ...
                    {effortMetric !== 'none' && (
                      <input
                        ...
                        disabled={isComplete}
                        className={`w-full bg-surface-raised rounded-lg px-1 py-2 text-[0.625rem] text-center outline-none ${
          ...
          {/* Justin's Log Set button */}
          {completedCount < setCount && (
            <button
              onClick={() => handleComplete(completedCount)}
              className="w-full mt-1 py-3 rounded-xl bg-accent-orange text-white text-sm font-bold flex items-center justify-center gap-2 shadow-md active:scale-[0.97] active:shadow-none transition-all"
              style={{ backgroundColor: '#e8572a' }}
            >
              <Check size={15} strokeWidth={3} />
              Log Set {completedCount + 1} / {setCount}
            </button>
          )}

          {/* Add/Remove set buttons */}
          <div className="flex items-center gap-2 pt-1 px-0.5 flex-wrap">
            <button
              onClick={addSet}
              className="text-[0.6875rem] text-text-muted hover:text-text-secondary px-2 py-1 rounded-lg bg-surface-raised"
            >
              + Set
            </button>
            {setCount > 1 && (
              <button
                onClick={removeSet}
                className="text-[0.6875rem] text-text-muted hover:text-danger px-2 py-1 rounded-lg bg-surface-raised"
              >
                - Set
              </button>
            )}
            <button
              ...
              className="text-[0.6875rem] text-warning hover:text-warning px-2 py-1 rounded-lg bg-warning/10"
            >
              + Warmup
            </button>
            {hasRemovableWarmup && (
              <button
                ...
                className="text-[0.6875rem] text-warning hover:text-danger px-2 py-1 rounded-lg bg-warning/10"
              >
                - Warmup
              </button>
            )}
            <button
              ...
              className="text-[0.6875rem] text-text-muted hover:text-text-secondary ml-auto"
            >
              {showNote ? '− Note' : '+ Note'}
            </button>
          </div>
          ...
          {showNote && (
            <input
              type="text"
              className="w-full bg-surface-raised rounded-lg px-3 py-2 text-[0.6875rem] text-text-secondary outline-none placeholder-text-muted"
          ...
          {lastPerformance && lastPerformance.sets.filter((s) => s.completed).length > 0 && (
            <p className="text-[0.625rem] text-text-muted px-0.5">
          ...
          {exercise.note && (
            <p className="text-[0.625rem] text-text-muted italic px-0.5">
              {exercise.note}
            </p>
          )}

          {exercise.flag && (
            <p className="text-[0.625rem] text-warning px-0.5">
              {exercise.flag}
            </p>
          )}

          {/* Swap + Video + Rename links */}
          <div className="flex gap-3 flex-wrap">
            <button onClick={() => setShowSwaps(!showSwaps)} className="text-[0.625rem] text-accent-blue font-medium">
              {showSwaps ? 'Hide alternatives' : 'Alternatives'}
            </button>
            {videoUrl ? (
              <a href={videoUrl} target="_blank" rel="noopener noreferrer" className="text-[0.625rem] text-accent font-medium">
                Watch demo
              </a>
            ) : (
              <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(exercise.name + ' exercise form')}`} target="_blank" rel="noopener noreferrer" className="text-[0.625rem] text-text-muted font-medium">
                Search YouTube
              </a>
            )}
            <button onClick={() => setShowVideoInput(!showVideoInput)} className="text-[0.625rem] text-text-muted font-medium">
              {videoUrl ? 'Edit video' : 'Add video'}
            </button>
            <button onClick={() => { setRenameValue(exercise.name); setShowRename(!showRename); }} className="text-[0.625rem] text-text-muted font-medium">
              Rename
            </button>
          </div>
          {showVideoInput && (
            <div className="flex gap-1">
              ...
              <button onClick={() => { updateExerciseVideo(exercise.name, videoUrl); setShowVideoInput(false); toast('Video saved', 'success'); }} className="bg-accent-blue text-white px-2 rounded-lg text-[0.625rem] font-semibold">Save</button>
            </div>
          )}
          {showRename && (
            <div className="flex gap-1">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && renameValue.trim()) {
                    onRename?.(exercise.id, renameValue.trim());
                    setShowRename(false);
                  }
                }}
              ...
              <button
                ...
                className="bg-accent-blue text-white px-2 rounded-lg text-[0.625rem] font-semibold"
              >
                Save
              </button>
            </div>
          ...
          {showSwaps && (
            <div className="space-y-2 mt-1">
                ...
                <div className="space-y-1">
                  {swapSuggestions.map((altName) => (
                    <div key={altName} className="flex items-center justify-between bg-surface-raised rounded-md px-2 py-1.5 gap-2">
                      <span className="text-[0.625rem] font-medium text-text-secondary">{altName}</span>
                      {onSwap && (
                        <button
                          onClick={() => { onSwap({ name: altName, muscles: [], equipment: '', type: 'compound' }); setShowSwaps(false); }}
                          className="shrink-0 text-[0.625rem] font-semibold text-accent-blue bg-accent-blue/10 hover:bg-accent-blue/20 px-2 py-0.5 rounded-md transition-colors"
                        >
                          Swap
                        </button>
                      )}
              ...
              <div className="border-t border-border-light pt-2 space-y-1.5">
                <p className="text-[0.5625rem] text-text-muted uppercase font-semibold tracking-wider">Use a different exercise</p>
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      setCustomAltInput(e.target.value);
                      setCustomAltSuggestions(e.target.value.trim().length >= 1 ? searchExerciseLibrary(e.target.value).slice(0, 5) : []);
                    }}
                  />
                  {customAltSuggestions.length > 0 && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-0.5 bg-surface border border-border rounded-xl shadow-lg overflow-hidden">
                      {customAltSuggestions.map((ex) => (
                        <button
                          key={ex.name}
                          type="button"
                          onMouseDown={(e) => e.preventDefault()}
                          onClick={() => { setCustomAltInput(ex.name); setCustomAltSuggestions([]); }}
                        ...
                        >
                          ...
                          <div className="text-[0.5625rem] text-text-muted">{ex.muscles.join(', ')} · {ex.equipment}</div>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                {customAltInput.trim() && (
                  <div className="flex gap-1.5">
                    {canSaveAlternative && onAddAlternative && onSwap && (
                      <button
                        onClick={() => {
                          const name = customAltInput.trim();
                          onAddAlternative(exercise.id, name);
                          onSwap({ name, muscles: [], equipment: '', type: 'compound' });
                          setCustomAltInput('');
                          setCustomAltSuggestions([]);
                          setShowSwaps(false);
                        }}
                        className="flex-1 py-1.5 rounded-lg bg-accent-blue text-white text-[0.625rem] font-semibold active:scale-95 transition-transform"
                      >
                        Use &amp; Save
                      </button>
                    )}
                    {onSwap && (
                      <button
                        onClick={() => {
                          onSwap({ name: customAltInput.trim(), muscles: [], equipment: '', type: 'compound' });
                          setCustomAltInput('');
                          setCustomAltSuggestions([]);
                          setShowSwaps(false);
                        }}
                        className="flex-1 py-1.5 rounded-lg bg-surface-raised text-text-secondary text-[0.625rem] font-semibold active:scale-95 transition-transform"
                      >
                        {canSaveAlternative ? 'Use once' : 'Use'}
                      </button>
                    )}
                  </div>
                )}
...
export function ActiveWorkout({
  ...
  // Session-level renames (don't touch the program)
  const [sessionRenames, setSessionRenames] = useState<Record<string, string>>({});

  function handleRename(exerciseId: string, newName: string) {
    setSessionRenames((prev) => ({ ...prev, [exerciseId]: newName }));
    setAddedExercises((prev) => prev.map((ex) => ex.id === exerciseId ? { ...ex, name: newName } : ex));
    toast(`Renamed to ${newName}`, 'success');
  }

  function handleAddAlternative(exerciseId: string, altName: string) {
    onAddAlternative?.(exerciseId, altName);
  }
  ...
  const [newExFlag, setNewExFlag] = useState('');
  const QUICK_FLAGS = ['Lengthened Partials', 'Superset', 'AMRAP', 'Drop Set', 'Myo-Reps', 'Rest-Pause', 'Paused Reps', 'Tempo'];

  const skippedIds = new Set(skippedExercises.map((s) => s.exerciseId));
  const removedIds = new Set(removedExerciseIds);
  const activeExercises = [
    ...day.exercises.filter((e) => !skippedIds.has(e.id) && !removedIds.has(e.id)).map((e) => sessionRenames[e.id] ? { ...e, name: sessionRenames[e.id] } : e),
    ...addedExercises.filter((e) => !removedIds.has(e.id)),
  ];

  function confirmRemoveExercise() {
    if (!removeTarget) return;
    const ex = day.exercises.find((e) => e.id === removeTarget) || addedExercises.find((e) => e.id === removeTarget);
    setRemovedExerciseIds((prev) => [...prev, removeTarget]);
    onRemoveExercise?.(removeTarget);
    toast(`Removed ${ex?.name || 'exercise'} from this session`, 'info');
  ...
  function handleAddExercise() {
    ...
    setNewExName(''); setNewExNamePicked(false); setNewExSets('3'); setNewExReps('10'); setNewExMuscle(''); setNewExInputType('reps'); setNewExFlag('');
  ...
  const handleComplete = useCallback(
    (exerciseId: string, weight: number, reps: number, rir?: number, rpe?: number, isWarmup?: boolean, setDuration?: number, setType?: SetType) => {
      ...
      if (uncheckedIdx >= 0) {
        onUpdateSet(exerciseId, uncheckedIdx, { weight, reps, completed: true, timestamp: Date.now(), rir, rpe, isWarmup, setType, ...(setDuration != null ? { duration: setDuration } : {}) });
        return;
      }
    ...
    <div className="pb-36">
      ...
      <div className="px-4 pt-4 space-y-3">
        {activeExercises.map((exercise, index) => {
              ...
              <div key={exercise.id} className="bg-surface rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {exercise.note && <div className="text-[0.625rem] text-text-muted italic mt-0.5">{exercise.note}</div>}
                  </div>
                  {existing && <Check size={16} className="text-success" />}
                </div>
                {existing ? (
                  <div className="flex gap-2 text-[0.6875rem] text-text-muted flex-wrap">
          ...
          return (
            <ExerciseCard
              ...
              canSaveAlternative={!!onSwapExercise}
        ...
        {skippedExercises.length > 0 && (
          <div className="bg-surface-raised/50 rounded-xl p-3 space-y-1.5">
            <span className="text-[0.625rem] font-medium text-text-muted uppercase tracking-wider">Skipped</span>
              ...
              return (
                <div key={s.exerciseId} className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    {s.reason && <span className="text-[0.625rem] text-text-muted italic">{s.reason}</span>}
                    <button onClick={() => undoSkip(s.exerciseId)} className="text-[0.625rem] text-accent-blue">Undo</button>
                  </div>
                </div>
              );
            })}
      ...
      {removeTarget && (
        <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-bg rounded-t-3xl w-full max-w-lg p-5 pb-20 space-y-3 animate-in slide-in-from-bottom">
            <p className="text-sm text-text-secondary">
              Remove "{activeExercises.find((e) => e.id === removeTarget)?.name}" from this session? Any sets logged for it here will be discarded. Your program stays unchanged.
            </p>
            <div className="flex gap-2 pt-1">
              <button onClick={() => setRemoveTarget(null)} className="btn-secondary flex-1 text-sm">Cancel</button>
              <button
                onClick={confirmRemoveExercise}
                className="flex-1 font-semibold rounded-xl px-6 py-3 active:scale-95 transition-transform bg-danger text-white text-sm"
              >
                Remove
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-bg rounded-t-3xl w-full max-w-lg p-5 pb-20 space-y-3 animate-in slide-in-from-bottom max-h-[80vh] overflow-y-auto">
            ...
            {!newExNamePicked && newExName.trim().length >= 2 && (
              <div className="space-y-1 max-h-36 overflow-y-auto">
                ...
                {searchCustomExercises(newExName).slice(0, 3).map((ex) => (
                  ...
                  >
                    <div className="text-xs font-medium flex items-center gap-1">
                      {ex.name}
                      <span className="text-[0.5rem] px-1 rounded bg-accent/15 text-accent">My Exercise</span>
                    </div>
                    {ex.muscle && <div className="text-[0.625rem] text-text-muted">{ex.muscle}</div>}
                  </button>
                ))}
                {searchExerciseLibrary(newExName).slice(0, 6).map((ex) => (
                  ...
                  >
                    <div className="text-xs font-medium">{ex.name}</div>
                    <div className="text-[0.625rem] text-text-muted">{ex.muscles.join(', ')} · {ex.equipment} · {ex.type}</div>
                  </button>
                ))}
              </div>
            )}
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Sets</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">{newExInputType === 'time' ? 'Target Time' : 'Reps'}</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Muscle</label>
            ...
            <div>
              <label className="text-[0.5625rem] text-text-muted mb-1 block">Input Type</label>
            ...
            <div>
              <label className="text-[0.5625rem] text-text-muted mb-1 block">Flag (optional)</label>
              <input
                type="text"
                className="input-field text-sm w-full"
                value={newExFlag}
                onChange={(e) => setNewExFlag(e.target.value)}
                placeholder="e.g. Superset, AMRAP"
              />
              <div className="flex flex-wrap gap-1 mt-1">
                {QUICK_FLAGS.map((f) => (
                  <button
                    key={f}
                    type="button"
                    onClick={() => setNewExFlag(f)}
                    className={`text-[0.5625rem] px-2 py-0.5 rounded-full border transition-colors ${
                      newExFlag === f ? 'bg-accent-orange/20 border-accent-orange text-accent-orange' : 'border-border text-text-muted'
                    }`}
                  >
                    {f}
                  </button>
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => { setShowAddExercise(false); setNewExName(''); setNewExNamePicked(false); setNewExInputType('reps'); setNewExFlag(''); }} className="btn-secondary flex-1 text-sm">Cancel</button>
        ...
        <div className="bg-surface rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="text-[0.6875rem] text-accent-blue font-medium flex items-center gap-1"
            >
              <Plus size={12} /> Add
            </button>
          </div>
              ...
              {cardioEntries.map((entry, idx) => (
                <div key={idx} className="flex items-center justify-between p-2.5 rounded-xl bg-surface-raised">
                  <div>
                    ...
                    <div className="flex gap-2 text-[0.6875rem] text-text-muted mt-0.5 flex-wrap">
                    ...
                    {entry.notes && <div className="text-[0.625rem] text-text-muted italic mt-0.5">{entry.notes}</div>}
                  </div>
      ...
      {showAddCardio && (
        <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-bg rounded-t-3xl w-full max-w-lg p-5 pb-20 space-y-3 animate-in slide-in-from-bottom">
            ...
            <div>
              <label className="text-[0.5625rem] text-text-muted mb-1 block">Type</label>
              <div className="flex flex-wrap gap-1.5 mb-1.5">
                {CARDIO_TYPES.map((t) => (
                  <button
                    key={t}
                    onClick={() => setCardioType(t)}
                    className={`px-2.5 py-1 rounded-lg text-[0.6875rem] font-medium transition-colors ${
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Duration (min) *</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Intensity</label>
                <div className="flex gap-1">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Avg HR (bpm)</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Distance</label>
                <div className="flex gap-1">
                  ...
                  <select className="input-field text-[0.625rem] py-0 w-12" value={cardioDistanceUnit} onChange={(e) => setCardioDistanceUnit(e.target.value as 'mi' | 'km')}>
              ...
              <div>
                <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Cal Burned</label>
            ...
            <div>
              <label className="text-[0.5625rem] text-text-muted mb-0.5 block">Notes</label>
