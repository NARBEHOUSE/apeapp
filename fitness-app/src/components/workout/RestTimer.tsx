...
export function RestTimer({ duration, onComplete, onDismiss }: Props) {
  ...
  const totalDuration = duration + extraSeconds;
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      const elapsed = Math.floor((Date.now() - startTime) / 1000);
      const r = Math.max(0, totalDuration - elapsed);
      setRemaining(r);
    }, 250);
  ...
  const addTime = (seconds: number) => {
    ...
    setExtraSeconds((prev) => prev + seconds);
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-[0.75rem] font-bold uppercase tracking-widest text-text-muted">Rest</p>
        ...
        <div className="flex gap-1">
          {[10, 30, 60].map((s) => (
            <button
              key={s}
              onClick={() => addTime(s)}
              className="flex-1 py-1 rounded-lg bg-surface-raised border border-border-light text-text-muted text-[0.625rem] font-semibold active:scale-95 transition-transform"
            >
              +{s}s
            </button>
          ))}
