    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4">
        ...
        >
          ...
          <span className={`text-[0.625rem] font-semibold mt-0.5 ${filter === 'all' ? 'text-accent-blue' : 'text-text-muted'}`}>All</span>
          <span className={`text-[0.5625rem] ${filter === 'all' ? 'text-accent-blue/70' : 'text-text-muted'}`}>{photos.length}</span>
        </button>
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-0 inset-x-0 px-1 pb-1 text-center">
                <div className="text-[0.625rem] font-bold text-white leading-tight">{pose.short}</div>
                <div className="text-[0.5625rem] text-white/70">{stat.count}</div>
              </div>
              {isActive && (
          ...
          {grouped.map(({ pose, photos: posePhotos }) => (
            <div key={pose.value}>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => setFilter(pose.value)}
                  className="text-[0.625rem] text-accent-blue"
                >
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex rounded-lg overflow-hidden border border-border-light">
                <button onClick={() => setShareFormat('post')} className={`px-2 py-1 text-[0.5625rem] font-semibold ${shareFormat === 'post' ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted'}`}>4:5</button>
                <button onClick={() => setShareFormat('story')} className={`px-2 py-1 text-[0.5625rem] font-semibold ${shareFormat === 'story' ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted'}`}>9:16</button>
              </div>
            </div>
            <div className="flex flex-wrap gap-1">
              {STAT_OPTIONS.filter((o) => o.value !== 'none').map((opt) => {
                ...
                return (
                  ...
                  }} className={`px-2 py-1 rounded-md text-[0.5625rem] font-semibold transition-colors ${selected ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted border border-border-light'}`}>
                    {opt.label}
                  </button>
                );
              })}
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent p-1.5">
        <p className="text-[0.5625rem] font-semibold text-white/90">{formatDate(photo.date)}</p>
        <p className="text-[0.5rem] text-white/60 uppercase">{POSE_LABELS[photo.pose] || photo.pose}</p>
      </div>
    </button>
  );
}
        ...
        >
          ...
          {photos.length > 1 && swipeHint && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-black/50 rounded-full px-3 py-1.5 pointer-events-none">
              ...
              <span className="text-[0.625rem] text-white/60 font-medium">swipe to navigate</span>
        ...
        {photos.length > 1 && !editing && (
          <div className="flex items-center justify-center pb-3 gap-1.5">
            {showDots ? (
              ...
              <span className="text-[0.6875rem] text-white/50 font-medium">{currentIdx + 1} / {photos.length}</span>
            )}
          </div>
        )}
        ...
        {editing && onUpdate && (
          <div className="px-4 pb-4 space-y-3 bg-black/60">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[0.5625rem] text-white/50 uppercase font-semibold">Date</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-white/50 uppercase font-semibold">Pose</label>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[0.5625rem] text-white/50 uppercase font-semibold">Weight</label>
              ...
              <div>
                <label className="text-[0.5625rem] text-white/50 uppercase font-semibold">Notes</label>
