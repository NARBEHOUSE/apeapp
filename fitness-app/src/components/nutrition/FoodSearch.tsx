...
import { searchSavedFoods, saveFoodToHistory, lookupByBarcode as lookupLocalBarcode, getCustomEmoji } from '../../db/foodHistory';
...
export function FoodSearch({ onAdd, onClose, profileId, defaultTab, saveOnly = false, multiMode = false, startWithScan = false, onSaveToLibrary, dailyTotals, macroTargets }: FoodSearchProps) {
  ...
  const [servingUnitInput, setServingUnitInput] = useState<ServingUnit>('g');
  ...
  const [editBaseUnit, setEditBaseUnit] = useState<ServingUnit>('g');
  ...
  const localResults = useMemo(() => {
    ...
    const historyResults: LocalResult[] = profileId
      ? searchSavedFoods(profileId, q).slice(0, 10).map((f) => ({
          ...
          source: f.source, isHistory: true, emoji: f.emoji,
        }))
      : [];
  ...
  // Custom emoji override (set in the food library) beats the auto-guessed one
  const resolveEmoji = useCallback((name: string, emoji?: string) => {
    return emoji || (profileId ? getCustomEmoji(profileId, name) : undefined) || getFoodEmoji(name);
  }, [profileId]);
  ...
  function selectLocal(item: LocalResult) {
    setSelected({
      ...
      commonServing: item.commonServing, emoji: item.emoji,
    });
    setServingSize(String(item.servingSize));
    setServingUnitInput(normaliseServingUnit(item.servingUnit));
    setServingsConsumed('1');
    setMealType('snack');
  }
  ...
  function handleSaveToLibrary() {
    ...
    const baseGrams = servingToGrams(selected.servingSize || 1, normaliseServingUnit(selected.servingUnit));
    const userGrams = servingToGrams(sz, servingUnitInput);
    const scaleFactor = baseGrams > 0 ? userGrams / baseGrams : 1;
    saveFoodToHistory(profileId, {
      name: selected.name,
      brand: selected.brand,
      servingSize: sz,
      servingUnit: servingUnitInput,
  ...
  function handleAdd() {
    ...
    // Auto-save to library if this food isn't already there
    if (profileId) {
      const alreadySaved = searchSavedFoods(profileId, selected.name).some(
        (f) => f.name.toLowerCase() === selected.name.toLowerCase()
      );
      if (!alreadySaved) {
        saveFoodToHistory(profileId, {
          name: selected.name,
    ...
    // Scale macros proportionally — convert both to grams so g↔oz changes work correctly
    const baseGrams = servingToGrams(selected.servingSize || 1, normaliseServingUnit(selected.servingUnit));
    const userGrams = servingToGrams(sz, servingUnitInput);
    const scaleFactor = baseGrams > 0 ? userGrams / baseGrams : 1;
    ...
    const finalUnit = servingUnitInput;
    if (multiMode) {
      setPlate((prev) => [...prev, {
        name: selected.name,
        brand: selected.brand,
  ...
  // Plate totals (computed here so it's available in the selected food detail view too)
  ...
  if (selected) {
    ...
    const myAmount = parseFloat(servingSize) || baseServing;
    const qty = parseFloat(servingsConsumed) || 1;

    // Scale macros in gram-equivalents so g↔oz unit changes work correctly
    const baseGrams = servingToGrams(baseServing || 1, normaliseServingUnit(selected.servingUnit));
    const myGrams = servingToGrams(myAmount, servingUnitInput);
    const factor = (baseGrams > 0 ? myGrams / baseGrams : 1) * qty;
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-2xl">{resolveEmoji(selected.name, selected.emoji)}</span>
          <div>
        ...
        {!editingBase ? (
          <div className="bg-surface-raised rounded-lg px-3 py-2 flex items-center justify-between">
            ...
            <button type="button" onClick={() => {
              ...
              setEditBaseUnit(normaliseServingUnit(selected.servingUnit));
            }} className="text-[0.5625rem] text-text-muted hover:text-accent-blue ml-2">Edit</button>
          </div>
        ) : (
          <div className="bg-surface-raised rounded-lg p-3 space-y-2 border border-warning/30">
            <div className="text-[0.5625rem] text-warning font-semibold">Editing base food — this saves as a custom copy</div>
            {/* Serving + unit row */}
            <div className="flex gap-1 items-end">
              <div className="flex-1">
                <label className="text-[0.5rem] text-text-muted block mb-0.5">Serving size</label>
                <input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editBaseServing} onChange={(e) => setEditBaseServing(e.target.value)} />
              </div>
              <div className="w-16">
                <label className="text-[0.5rem] text-text-muted block mb-0.5">Unit</label>
                <select
                  className="input-field text-xs w-full py-1"
                  value={editBaseUnit}
                  onChange={(e) => {
                    const newUnit = e.target.value as ServingUnit;
                    const converted = convertServingUnit(parseFloat(editBaseServing) || 0, editBaseUnit, newUnit);
                    setEditBaseUnit(newUnit);
                    setEditBaseServing(String(converted));
                  }}
                >
                  {SERVING_UNITS.map((u) => (
            ...
            <div className="grid grid-cols-4 gap-1">
              <div><label className="text-[0.5rem] text-text-muted">Cal</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editBaseCal} onChange={(e) => setEditBaseCal(e.target.value)} /></div>
              <div><label className="text-[0.5rem] text-text-muted">Prot</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editBaseP} onChange={(e) => setEditBaseP(e.target.value)} /></div>
              <div><label className="text-[0.5rem] text-text-muted">Carbs</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editBaseC} onChange={(e) => setEditBaseC(e.target.value)} /></div>
              <div><label className="text-[0.5rem] text-text-muted">Fat</label><input type="text" inputMode="decimal" className="input-field text-xs w-full py-1" value={editBaseF} onChange={(e) => setEditBaseF(e.target.value)} /></div>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => setEditingBase(false)} className="flex-1 py-1 rounded-md bg-surface text-[0.625rem] text-text-muted font-medium">Cancel</button>
              <button type="button" onClick={() => {
                ...
                setSelected({ ...selected, calories: newCal, protein: newP, carbs: newC, fat: newF, servingSize: newServing, servingUnit: editBaseUnit, source: 'manual' });
                setServingSize(String(newServing));
                setServingUnitInput(editBaseUnit);
                if (profileId) {
                  saveFoodToHistory(profileId, {
                    name: selected.name, brand: selected.brand, calories: newCal, protein: newP,
                    carbs: newC, fat: newF, servingSize: newServing, servingUnit: editBaseUnit, source: 'manual',
                  });
                }
                setEditingBase(false);
              }} className="flex-1 py-1 rounded-md bg-accent-blue text-white text-[0.625rem] font-semibold">Save as Custom</button>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex gap-1">
              ...
              <select
                ...
                onChange={(e) => {
                  const newUnit = e.target.value as ServingUnit;
                  const converted = convertServingUnit(parseFloat(servingSize) || 0, servingUnitInput, newUnit);
                  setServingUnitInput(newUnit);
            ...
            {selected.commonServing && (
              <button
                ...
                className="text-[0.625rem] text-accent-blue mt-1"
              >
                Log as: {selected.commonServing.label} ({selected.commonServing.grams}g)
              </button>
            )}
          </div>
          <div>
        ...
        <div className="bg-surface rounded-xl p-3 grid grid-cols-4 gap-2 text-center">
          <div><div className="text-lg font-bold text-accent-orange">{dispCal}</div><div className="text-[0.5625rem] text-text-muted">kcal</div></div>
          <div><div className="text-lg font-bold text-accent-blue">{dispP}g</div><div className="text-[0.5625rem] text-text-muted">protein</div></div>
          <div><div className="text-lg font-bold text-success">{dispC}g</div><div className="text-[0.5625rem] text-text-muted">carbs</div></div>
          <div><div className="text-lg font-bold text-nutrition">{dispF}g</div><div className="text-[0.5625rem] text-text-muted">fat</div></div>
        </div>

        {/* Remaining today indicator */}
        {!saveOnly && dailyTotals && macroTargets && (
          <div className="bg-surface rounded-xl px-3 py-2">
            <div className="text-[0.5625rem] text-text-muted font-semibold uppercase mb-1.5">Remaining today after this</div>
            <div className="grid grid-cols-4 gap-1 text-center">
              {[
                { label: 'kcal', remaining: macroTargets.calories - dailyTotals.calories - plateTotals.cal - dispCal, color: 'text-accent-orange' },
                { label: 'protein', remaining: macroTargets.protein - dailyTotals.protein - plateTotals.p - dispP, color: 'text-accent-blue' },
                { label: 'carbs', remaining: macroTargets.carbs - dailyTotals.carbs - plateTotals.c - dispC, color: 'text-success' },
                { label: 'fat', remaining: macroTargets.fat - dailyTotals.fat - plateTotals.f - dispF, color: 'text-nutrition' },
              ].map(({ label, remaining, color }) => {
                const isOver = remaining < 0;
                return (
                  <div key={label}>
                    <div className={`text-sm font-bold ${isOver ? 'text-danger' : color}`}>
                      {isOver && <span className="text-[0.625rem] mr-0.5">⚠</span>}
                      {Math.round(remaining)}{label !== 'kcal' ? 'g' : ''}
                    </div>
                    <div className="text-[0.5rem] text-text-muted">{label}</div>
                  </div>
                );
              })}
            </div>
        ...
        {!saveOnly && (
          <p className="text-[0.5rem] text-text-muted text-center -mt-1">Adding to {multiMode ? 'plate' : 'log'} also saves to your library</p>
        )}
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {showScanner && (
        <div className="space-y-2">
          ...
          <p className="text-[0.625rem] text-text-muted text-center">Point camera at barcode</p>
        </div>
      )}
      ...
      {barcodeResult && (
        <button type="button" onClick={() => selectUsda(barcodeResult)}
          ...
          <span className="text-xl shrink-0">{resolveEmoji(barcodeResult.name)}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-medium truncate">{barcodeResult.name}</span>
              <span className="text-[0.5rem] px-1 py-0.5 rounded bg-green-500/15 text-green-500 shrink-0">Barcode</span>
            </div>
            {barcodeResult.brand && <div className="text-[0.625rem] text-text-muted truncate">{barcodeResult.brand}</div>}
      ...
      {barcodeError && !showManualForBarcode && (
        <div className="space-y-2">
          <p className="text-[0.625rem] text-danger text-center">{barcodeError}</p>
          {profileId && (
            <button
              type="button"
      ...
      <div className="space-y-1 max-h-64 overflow-y-auto">
        {localResults.map((item, i) => (
          <button key={`local-${item.name}-${i}`} type="button" onClick={() => selectLocal(item)}
            ...
            <span className="text-base shrink-0">{resolveEmoji(item.name, item.emoji)}</span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-medium truncate">{item.name}</span>
                {item.isHistory && <span className="text-[0.5625rem] px-1 py-0.5 rounded bg-surface text-text-muted shrink-0 flex items-center gap-0.5"><Clock size={8} /> Recent</span>}
                {item.category && !item.isHistory && <span className="text-[0.5625rem] px-1 py-0.5 rounded bg-surface text-text-muted shrink-0">{item.category}</span>}
              </div>
        ...
        {usdaResults.length > 0 && (
          <>
            {localResults.length > 0 && <div className="text-[0.5625rem] text-text-muted font-semibold uppercase tracking-wider px-2 pt-2">USDA Database</div>}
              ...
              <button key={food.fdcId} type="button" onClick={() => selectUsda(food)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[0.5rem] px-1 py-0.5 rounded bg-green-500/15 text-green-500 shrink-0">{food.source === 'off' ? 'OFF' : 'USDA'}</span>
                  </div>
                  {food.brand && <div className="text-[0.625rem] text-text-muted truncate">{food.brand}</div>}
                  <div className="text-[0.625rem] text-text-muted mt-0.5">
      ...
      {usdaError && <p className="text-[0.625rem] text-danger text-center">{usdaError}</p>}

      {!query.trim() && !showScanner && !barcodeResult && (
        <p className="text-[0.6875rem] text-text-muted text-center">Search {FOOD_DATABASE.length}+ foods, USDA database, or scan a barcode</p>
      )}
    </div>
  );
}
