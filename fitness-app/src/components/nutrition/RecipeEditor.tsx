  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          <label className="text-[0.625rem] text-text-muted font-semibold uppercase">Servings</label>
          <input type="number" className="input-field text-sm w-full mt-1" value={servings} onChange={(e) => setServings(e.target.value)} min="1" />
        </div>
        <div>
          <label className="text-[0.625rem] text-text-muted font-semibold uppercase">Prep (min)</label>
        ...
        <div>
          <label className="text-[0.625rem] text-text-muted font-semibold uppercase">Cook (min)</label>
      ...
      <div>
        ...
        {hasIngredientMacros ? (
          ...
          <div className="grid grid-cols-4 gap-1.5">
            <div><label className="text-[0.5625rem] text-text-muted">Calories</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" placeholder="0" value={manualCal} onChange={(e) => setManualCal(e.target.value)} /></div>
            <div><label className="text-[0.5625rem] text-text-muted">Protein</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" placeholder="0" value={manualP} onChange={(e) => setManualP(e.target.value)} /></div>
            <div><label className="text-[0.5625rem] text-text-muted">Carbs</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" placeholder="0" value={manualC} onChange={(e) => setManualC(e.target.value)} /></div>
            <div><label className="text-[0.5625rem] text-text-muted">Fat</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" placeholder="0" value={manualF} onChange={(e) => setManualF(e.target.value)} /></div>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => { setShowIngForm(true); setIngMode('search'); }} className="text-[0.625rem] text-accent-blue font-semibold flex items-center gap-0.5">
            <Plus size={10} /> Add
          </button>
        </div>
              ...
              if (isEditing) {
                return (
                  <div key={i} className="bg-surface rounded-xl p-3 space-y-2 border border-accent-blue/30">
                      ...
                      <div className="space-y-1 max-h-28 overflow-y-auto">
                        {editIngSearchResults.map((food, fi) => (
                          <button key={fi} onClick={() => {
                            ...
                            <span className={`text-[0.5rem] px-1 rounded ml-1 ${food.source === 'builtin' ? 'bg-accent-blue/15 text-accent-blue' : 'bg-surface text-text-muted'}`}>{food.source === 'builtin' ? 'DB' : 'My Foods'}</span>
                    ...
                    {/* USDA search button */}
                    {(
                      <button onClick={handleUsdaSearch} disabled={usdaSearching} className="text-[0.625rem] text-accent-blue font-semibold disabled:opacity-50">
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1"><label className="text-[0.5625rem] text-text-muted">Amount</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={editIngAmount} onChange={(e) => setEditIngAmount(e.target.value)} /></div>
                      <div className="w-16"><label className="text-[0.5625rem] text-text-muted">Unit</label><select className="input-field text-xs w-full" value={editIngUnit} onChange={(e) => setEditIngUnit(e.target.value)}>{UNIT_OPTIONS.map((u) => <option key={u} value={u}>{u}</option>)}</select></div>
                    </div>
                    <div className="grid grid-cols-4 gap-1">
                      <div><label className="text-[0.5625rem] text-text-muted">Cal</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={editIngCal} onChange={(e) => setEditIngCal(e.target.value)} /></div>
                      <div><label className="text-[0.5625rem] text-text-muted">Prot</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={editIngP} onChange={(e) => setEditIngP(e.target.value)} /></div>
                      <div><label className="text-[0.5625rem] text-text-muted">Carbs</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={editIngC} onChange={(e) => setEditIngC(e.target.value)} /></div>
                      <div><label className="text-[0.5625rem] text-text-muted">Fat</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={editIngF} onChange={(e) => setEditIngF(e.target.value)} /></div>
                    </div>
                    <div className="flex gap-2">
        ...
        {ingredients.length === 0 && !showIngForm && (
          <p className="text-[0.6875rem] text-text-muted mb-3">No ingredients yet. Add them to auto-calculate macros, or enter total macros above.</p>
        )}
          ...
          <div className="bg-surface rounded-xl p-3 space-y-2 border border-border-light">
            ...
            <div className="flex gap-1 mb-1">
              <button
                ...
                className={`flex-1 py-1 rounded-md text-[0.625rem] font-semibold transition-colors ${ingMode === 'search' ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted'}`}
              >
                <Search size={10} className="inline mr-1" />Search Foods
              </button>
              <button
                onClick={() => setIngMode('manual')}
                className={`flex-1 py-1 rounded-md text-[0.625rem] font-semibold transition-colors ${ingMode === 'manual' ? 'bg-accent-blue text-white' : 'bg-surface-raised text-text-muted'}`}
              >
                Manual Entry
              </button>
            </div>
            ...
            {ingMode === 'search' && (
              <div>
                    ...
                    {searchResults.map((food, i) => (
                      ...
                      >
                        <div className="text-xs font-medium truncate flex items-center gap-1">
                          {food.name}
                          <span className={`text-[0.5rem] px-1 rounded ${food.source === 'builtin' ? 'bg-accent-blue/15 text-accent-blue' : 'bg-surface text-text-muted'}`}>
                ...
                {ingQuery.trim() && searchResults.length === 0 && (
                  <p className="text-[0.625rem] text-text-muted mt-1">No results. <button onClick={() => { setIngName(ingQuery); setIngMode('manual'); }} className="text-accent-blue font-semibold">Enter manually</button></p>
                )}
              </div>
            )}
            ...
            {ingMode === 'manual' && (
              <>
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  <div><label className="text-[0.5625rem] text-text-muted">Cal</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={ingCal} onChange={(e) => setIngCal(e.target.value)} /></div>
                  <div><label className="text-[0.5625rem] text-text-muted">Protein</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={ingP} onChange={(e) => setIngP(e.target.value)} /></div>
                  <div><label className="text-[0.5625rem] text-text-muted">Carbs</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={ingC} onChange={(e) => setIngC(e.target.value)} /></div>
                  <div><label className="text-[0.5625rem] text-text-muted">Fat</label><input type="number" inputMode="decimal" className="input-field text-xs w-full" value={ingF} onChange={(e) => setIngF(e.target.value)} /></div>
                </div>
                <div className="flex gap-2">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={addStep} className="text-[0.625rem] text-accent-blue font-semibold flex items-center gap-0.5">
        ...
        <div className="space-y-2">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-2 items-start">
              <span className="text-[0.625rem] text-text-muted font-bold mt-2.5 w-4 text-right">{i + 1}</span>
              <textarea
      ...
      {/* Tags */}
      <div>
        <label className="text-[0.625rem] text-text-muted font-semibold uppercase">Tags (comma-separated)</label>
