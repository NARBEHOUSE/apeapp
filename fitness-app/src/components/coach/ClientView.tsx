            ...
            <div className="space-y-2">
              ...
              {myPrograms.map((prog) => (
                <button key={prog.id} onClick={() => { setShowNewProgramMenu(false); setEditingProgram({ ...prog, id: crypto.randomUUID(), isBuiltIn: false, name: `${prog.name} (for ${data.profile.name || 'client'})` }); }} className="w-full text-left p-3 rounded-xl bg-surface-raised hover:bg-surface transition-colors">
                  ...
                  <div className="text-[0.625rem] text-text-muted">{prog.days.length} days · {prog.days.reduce((a, d) => a + d.exercises.length, 0)} exercises</div>
                </button>
              ))}
            </div>
          )}
    ...
    <div className="min-h-screen bg-bg">
      <div className="sticky top-0 z-30 bg-accent-blue text-white px-4 py-2 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[0.625rem] opacity-70">
            {lastRefreshed
      ...
      <div className="flex border-b border-border bg-surface sticky top-[52px] z-20 overflow-x-auto">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 py-2.5 text-[0.625rem] font-medium flex flex-col items-center gap-0.5 transition-colors min-w-[55px] ${tab === t.key ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-text-muted'}`}
          >
      ...
      {['overview','workouts','nutrition','progress','checkins'].includes(tab) && (
        <div className="sticky top-[94px] z-10 bg-bg px-4 pt-2 pb-1 flex items-center justify-between">
          <span className="text-[0.625rem] text-text-muted uppercase tracking-wider">Range</span>
          <div className="flex gap-1.5">
            {([7, 30, 60, 90] as const).map((r) => (
              <button key={r} onClick={() => setChartRange(r)}
                className={`px-2.5 py-1 rounded-full text-[0.6875rem] font-medium transition-colors ${chartRange === r ? 'bg-text-primary text-bg' : 'bg-surface text-text-muted'}`}
              >
                {r}d
              </button>
            ))}
      ...
      <div className="p-4 space-y-4 pb-24">
          ...
          <>
            ...
            <div className="card p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                {data.profile.bodyStats && (
                  <div className="text-right text-[0.625rem] text-text-muted">
                ...
                <div className="flex gap-2 pt-1 border-t border-border">
                  {[
                    ...
                    <div key={m.label} className="flex-1 text-center bg-surface-raised rounded-lg py-1.5">
                      ...
                      <div className="text-[0.5625rem] text-text-muted">{m.label}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="card p-3 space-y-0.5">
                <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Workouts</div>
                <div className="text-2xl font-bold" style={{ color: '#e8572a' }}>{rangeStats.workoutsInRange}</div>
                <div className="text-[0.625rem] text-text-muted">in {chartRange} days</div>
              </div>
              {/* Weight change */}
              <div className="card p-3 space-y-0.5">
                <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Weight</div>
                {rangeStats.avgWeight != null ? (
                  <>
                    ...
                    {rangeStats.weightDelta != null && (
                      <div className={`text-[0.6875rem] font-medium ${rangeStats.weightDelta < 0 ? 'text-success' : rangeStats.weightDelta > 0 ? 'text-danger' : 'text-text-muted'}`}>
              ...
              {/* Avg calories */}
              <div className="card p-3 space-y-0.5">
                <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Avg Calories</div>
                {rangeStats.avgCal != null ? (
                  <>
                    ...
                    {rangeStats.calTarget && rangeStats.calHit != null && (
                      <div className="text-[0.625rem] text-text-muted">On target {rangeStats.calHit}/{rangeStats.loggedDays} days</div>
                    )}
                  </>
                ) : <div className="text-sm text-text-muted">No data</div>}
              </div>
              {/* Avg protein */}
              <div className="card p-3 space-y-0.5">
                <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Avg Protein</div>
                {rangeStats.avgProtein != null ? (
                  <>
                    ...
                    {rangeStats.proteinTarget && rangeStats.proteinHit != null && (
                      <div className="text-[0.625rem] text-text-muted">Hit target {rangeStats.proteinHit}/{rangeStats.loggedDays} days</div>
                    )}
                  </>
                ) : <div className="text-sm text-text-muted">No data</div>}
              </div>
              {/* Nutrition logged */}
              <div className="card p-3 space-y-0.5">
                <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Nutrition Logged</div>
                <div className="text-2xl font-bold" style={{ color: '#f5a623' }}>{rangeStats.loggedDays}</div>
                <div className="text-[0.625rem] text-text-muted">of {chartRange} days</div>
              </div>
              {/* Steps */}
              {rangeStats.avgSteps != null ? (
                <div className="card p-3 space-y-0.5">
                  <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Avg Steps</div>
                  <div className="text-2xl font-bold" style={{ color: '#2e9e6b' }}>{rangeStats.avgSteps.toLocaleString()}</div>
                  <div className="text-[0.625rem] text-text-muted">per day</div>
                </div>
              ) : (
                <div className="card p-3 space-y-0.5">
                  <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Weigh-ins</div>
              ...
              {rangeStats.avgWater != null && (
                <div className="card p-3 space-y-0.5">
                  <div className="text-[0.5625rem] text-text-muted uppercase tracking-wider">Avg Water</div>
                  <div className="text-2xl font-bold" style={{ color: '#38bdf8' }}>{rangeStats.avgWater}</div>
                  <div className="text-[0.625rem] text-text-muted">{rangeStats.waterUnit}/day</div>
                </div>
              )}
            </div>
                ...
                <div className="h-32">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={weightChartData}>
                      <XAxis dataKey="date" tick={{ fontSize: Math.round(9 * fontScale) }} interval="preserveStartEnd" />
                      <YAxis tick={{ fontSize: Math.round(9 * fontScale) }} width={32} domain={['auto', 'auto']} />
                      <Tooltip contentStyle={{ fontSize: Math.round(11 * fontScale), background: '#1a1a1f', border: '1px solid #333', borderRadius: 8 }} formatter={(v) => [`${v} lbs`, 'Weight']} />
            ...
            {responses && responses.responses.some((r) => r.action === 'denied') && (
              <div className="card p-4 space-y-2 border-l-4 border-l-danger">
                ...
                <div className="text-[0.625rem] text-text-muted">{new Date(responses.respondedAt).toLocaleString()}</div>
                ...
                <button onClick={async () => { await onAcknowledgeResponse(fileId); setResponses(null); toast('Dismissed', 'success'); }} className="text-[0.625rem] text-text-muted underline">Dismiss</button>
              </div>
            )}
          </>
        )}
          ...
          <>
            {workoutFrequencyData.some((w) => w.workouts > 0) && (
              <div className="card p-4">
                ...
                <div className="text-[0.625rem] text-text-muted mb-3">{rangeStats.workoutsInRange} workouts in {chartRange} days</div>
                <SVGBarChart
                  key={chartRange}
                ...
                <div key={w.id} className="card overflow-hidden">
                  ...
                  {isExp && (
                    <div className="px-3 pb-3 space-y-1.5 border-t border-border pt-2">
                      ...
                      {w.notes && <div className="text-[0.625rem] text-text-muted italic mt-1">{w.notes}</div>}
                    </div>
                  )}
                </div>
              );
          ...
          <>
            {nutritionTrendData.length >= 3 && (
              <div className="card p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {rangeStats.avgCal != null && (
                    <div className="text-[0.625rem] text-text-muted">avg {rangeStats.avgCal.toLocaleString()} cal · {rangeStats.avgProtein}g protein</div>
                  )}
                </div>
                {/* Calorie trend */}
                <div>
                  <div className="text-[0.625rem] text-text-muted mb-1">Calories vs Target</div>
                  <SVGBarChart
                    key={`cal-${chartRange}`}
                    data={nutritionTrendData.map((d) => ({ label: d.date, value: d.cal }))}
                    color="#e8572a"
                ...
                {/* Protein trend */}
                <div>
                  <div className="text-[0.625rem] text-text-muted mb-1">Daily Protein (g)</div>
                  <div className="h-28">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={nutritionTrendData}>
                        <XAxis dataKey="date" tick={{ fontSize: Math.round(9 * fontScale) }} interval="preserveStartEnd" />
                        <YAxis tick={{ fontSize: Math.round(9 * fontScale) }} width={28} />
                        <Tooltip contentStyle={{ fontSize: Math.round(11 * fontScale), background: '#1a1a1f', border: '1px solid #333', borderRadius: 8 }} />
                ...
                {/* Carbs + fat */}
                <div>
                  <div className="text-[0.625rem] text-text-muted mb-1">Carbs &amp; Fat (g)</div>
                  <div className="h-28">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={nutritionTrendData}>
                        <XAxis dataKey="date" tick={{ fontSize: Math.round(9 * fontScale) }} interval="preserveStartEnd" />
                  ...
                  <div className="flex gap-3 mt-1">
                    <span className="text-[0.5625rem] flex items-center gap-1"><span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: '#2e9e6b' }} />Carbs</span>
                    <span className="text-[0.5625rem] flex items-center gap-1"><span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: '#f5a623' }} />Fat</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="text-center">
                ...
                {foodDates.length > 0 && (
                  <div className="text-[0.625rem] text-text-muted">{dateIdx + 1} of {foodDates.length} logged days</div>
                )}
              </div>
              <button
                      ...
                      return (
                        <div key={mealType}>
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[0.6875rem] font-semibold uppercase tracking-wider text-text-secondary capitalize">{mealType}</span>
                            <span className="text-[0.6875rem] text-text-muted">{mealCal} cal</span>
                          </div>
                              ...
                              return (
                                <div key={f.id} className="card p-3 flex items-center gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex gap-3 text-[0.625rem] mt-0.5 flex-wrap">
                                  ...
                                  <div className="text-right flex-shrink-0">
                                    ...
                                    <div className="text-[0.5625rem] text-text-muted">cal</div>
                                  </div>
                                </div>
                              );
                            })}
                  ...
                  return (
                    <div key={m.id} className="card p-3 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {delta !== null && (
                          <span className={`text-[0.625rem] font-medium ${delta < 0 ? 'text-success' : delta > 0 ? 'text-danger' : 'text-text-muted'}`}>
                  ...
                  <div className="h-44">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={bodyMeasurementsData}>
                        <XAxis dataKey="date" tick={{ fontSize: Math.round(9 * fontScale) }} interval="preserveStartEnd" />
                        <YAxis tick={{ fontSize: Math.round(9 * fontScale) }} width={28} domain={['auto', 'auto']} />
                        <Tooltip contentStyle={{ fontSize: Math.round(11 * fontScale), background: '#1a1a1f', border: '1px solid #333', borderRadius: 8 }} />
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
                    {[['waist','#e8572a'],['chest','#5b6ef5'],['hips','#2e9e6b'],['arms','#f5a623']].map(([k,c]) =>
                      bodyMeasurementsData.some((d) => d[k as keyof typeof d] != null) ? (
                        <span key={k} className="text-[0.5625rem] flex items-center gap-1">
            ...
            <div className="space-y-4">
                ...
                <div className="card p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[0.625rem] text-success font-medium mt-0.5">Currently enrolled</div>
                    </div>
                  </div>
                  {enrolled.days.map((day) => (
                    <div key={day.id} className="pl-3 border-l-2 border-border space-y-0.5">
                      ...
                      {day.exercises.map((ex, i) => <div key={i} className="text-[0.625rem] text-text-muted">{ex.name} — {ex.sets}×{ex.reps}</div>)}
                    </div>
                  ))}
                  {!readonly && (
              ...
              {pendingProgram && (
                <div className="card p-3 border-2 border-accent-blue/30 space-y-1">
                  <div className="text-[0.5625rem] text-accent-blue uppercase font-semibold tracking-wider">Staged for push</div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setEditingProgram(pendingProgram)} className="text-[0.625rem] text-accent-blue font-medium">Edit before pushing</button>
                    <button onClick={() => setPendingProgram(null)} className="text-[0.625rem] text-danger font-medium">Remove</button>
                  </div>
                </div>
              )}
        ...
        {/* CHECK-INS */}
        {tab === 'checkins' && (<>{checkInTrend.length >= 2 && <div className="card p-4 space-y-3"><div className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Check-In Trends</div><div className="h-48"><ResponsiveContainer width="100%" height="100%"><LineChart data={checkInTrend}><XAxis dataKey="date" tick={{ fontSize: Math.round(9 * fontScale) }} interval="preserveStartEnd" /><YAxis domain={[1, 10]} tick={{ fontSize: Math.round(9 * fontScale) }} width={20} /><Tooltip contentStyle={{ fontSize: Math.round(11 * fontScale), background: '#1a1a1f', border: '1px solid #333', borderRadius: 8 }} />{DEFAULT_CHECKIN_QUESTIONS.map((q, i) => <Line key={q.id} type="monotone" dataKey={q.id} name={q.label} stroke={CHART_COLORS[i % CHART_COLORS.length]} strokeWidth={1.5} dot={false} connectNulls />)}</LineChart></ResponsiveContainer></div><div className="flex flex-wrap gap-x-3 gap-y-1">{DEFAULT_CHECKIN_QUESTIONS.map((q, i) => <span key={q.id} className="text-[0.5625rem] flex items-center gap-1"><span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: CHART_COLORS[i % CHART_COLORS.length] }} />{q.label}</span>)}</div></div>}{(!data.checkIns || data.checkIns.length === 0) ? <p className="text-sm text-text-muted text-center py-8">No check-ins</p> : <div className="space-y-2"><div className="text-xs font-semibold text-text-secondary uppercase tracking-wider">History</div>{[...data.checkIns].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 21).map((ci) => <div key={ci.id} className="card p-3 space-y-2"><div className="text-xs font-semibold">{ci.date}</div><div className="space-y-1.5">{ci.responses.map((r) => { const q = DEFAULT_CHECKIN_QUESTIONS.find((qq) => qq.id === r.questionId); return <div key={r.questionId} className="flex items-center justify-between"><span className="text-xs text-text-secondary">{q?.label || r.questionId}</span>{typeof r.value === 'number' ? <div className="flex items-center gap-1"><div className="w-16 h-1.5 rounded-full bg-surface-raised overflow-hidden"><div className="h-full rounded-full" style={{ width: `${(r.value / 10) * 100}%`, backgroundColor: r.value >= 7 ? '#2e9e6b' : r.value >= 4 ? '#f5a623' : '#e85757' }} /></div><span className="text-xs font-medium w-5 text-right">{r.value}</span></div> : <span className="text-xs text-text-muted">{r.value}</span>}</div>; })}</div>{ci.notes && <p className="text-[0.625rem] text-text-muted italic">{ci.notes}</p>}</div>)}</div>}</>)}

        {/* CHANGES — unified push tab */}
        {tab === 'changes' && !readonly && (
          <div className="space-y-4">
            <p className="text-[0.6875rem] text-text-muted">Make all your changes here, then push everything to the client at once.</p>
            ...
            <div className="card p-4 space-y-3">
              ...
              {changeMacros && <>
                <div className="bg-surface-raised rounded-xl p-3 text-center"><div className="text-xl font-semibold">{editCalories}</div><div className="text-[0.5625rem] text-text-muted">calories</div></div>
                <div className="grid grid-cols-3 gap-2">
                  <div><label className="text-[0.5625rem] text-text-muted block mb-0.5">Protein</label><input type="number" inputMode="numeric" className="input-field text-sm text-center" value={editProtein} onChange={(e) => setEditProtein(e.target.value)} /></div>
                  <div><label className="text-[0.5625rem] text-text-muted block mb-0.5">Carbs</label><input type="number" inputMode="numeric" className="input-field text-sm text-center" value={editCarbs} onChange={(e) => setEditCarbs(e.target.value)} /></div>
                  <div><label className="text-[0.5625rem] text-text-muted block mb-0.5">Fat</label><input type="number" inputMode="numeric" className="input-field text-sm text-center" value={editFat} onChange={(e) => setEditFat(e.target.value)} /></div>
                </div>
            ...
            <div className="card p-4 space-y-3">
              ...
              {changeQuestions && <>
                <div className="space-y-1.5">
                  {coachQuestions.map((q) => <div key={q.id} className="flex items-center gap-2 p-2 rounded-lg bg-surface-raised"><span className="text-xs flex-1">{q.label}</span><span className="text-[0.5625rem] text-text-muted">1-10</span><button onClick={() => setCoachQuestions((prev) => prev.filter((x) => x.id !== q.id))} className="p-0.5 text-text-muted hover:text-danger"><X size={12} /></button></div>)}
                </div>
            ...
            <div className="card p-4 space-y-3">
              ...
              {pendingProgram ? (
                <div className="p-3 rounded-xl bg-success/10 border border-success/30 space-y-1">
                  ...
                  <div className="text-xs text-text-muted">{pendingProgram.days.length} days · {pendingProgram.days.reduce((a, d) => a + d.exercises.length, 0)} exercises</div>
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setEditingProgram(pendingProgram)} className="text-[0.625rem] text-accent-blue font-medium">Edit</button>
                    <button onClick={() => setPendingProgram(null)} className="text-[0.625rem] text-danger font-medium">Remove</button>
                  </div>
    ...
    <div className="space-y-3 mt-4">
      <div className="flex items-center gap-2">
        <div className="text-xs font-semibold text-text-secondary uppercase tracking-wider flex-1">
          Progress Photos {photosLoading && <span className="text-text-muted text-[0.625rem] font-normal">(loading…)</span>}
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4">
        ...
        >
          <span className={`text-[0.625rem] font-semibold ${poseFilter === 'all' ? 'text-accent-blue' : 'text-text-muted'}`}>All</span>
          <span className={`text-[0.5625rem] ${poseFilter === 'all' ? 'text-accent-blue/70' : 'text-text-muted'}`}>{photoMeta.length}</span>
        </button>
        {poseGroups.map((g) => (
          <button
            key={g.value}
          ...
          >
            ...
            <div className="absolute bottom-0 inset-x-0 px-1 pb-0.5 text-center">
              <div className="text-[0.5625rem] font-bold text-white">{g.label}</div>
              <div className="text-[0.5rem] text-white/60">{g.photos.length}</div>
            </div>
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {poseGroups.map((g) => {
            ...
            return (
              <div key={g.value}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[0.6875rem] font-semibold text-text-secondary uppercase tracking-wider">{g.label}</span>
                  {g.photos.length > 3 && (
                    <button onClick={() => setPoseFilter(g.value)} className="text-[0.625rem] text-accent-blue">
                    ...
                    <div key={p.photoId} className="flex flex-col gap-0.5">
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent px-1 pb-1">
                          <div className="text-[0.5rem] text-white/90 font-medium">{p.date}</div>
                        </div>
                      </button>
                      {p.weight != null && <div className="text-[0.5625rem] text-text-muted text-center">{p.weight} lbs</div>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      >
        ...
        {photos.length > 1 && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-black/40 rounded-full px-3 py-1.5">
            ...
            <span className="text-[0.625rem] text-white/50">swipe</span>
      ...
      {photos.length > 1 && (
        <div className="flex items-center justify-center pb-4 gap-1.5">
          {showDots ? (
            ...
            <span className="text-[0.6875rem] text-white/50">{currentIdx + 1} / {photos.length}</span>
          )}
        </div>
      )}
    </div>
