  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleExport} className="flex items-center gap-1 text-[0.625rem] text-accent-blue font-medium">
          ...
          <div key={entry.id} className="card overflow-hidden">
            ...
            {isExpanded && (
              <div className="px-3 pb-3 space-y-1 border-t border-border pt-2">
                {entry.coachEmail && (
                  <div className="text-[0.625rem] text-text-muted mb-1">{entry.coachEmail}</div>
                )}
                {entry.items.map((item, i) => (
                  <div key={i} className={`flex items-start gap-2 p-2 rounded-lg text-xs ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.coachNote && (
                        <div className="text-[0.625rem] text-text-muted mt-0.5">Coach: &ldquo;{item.coachNote}&rdquo;</div>
                      )}
                      {item.clientNote && (
                        <div className="text-[0.625rem] text-text-muted mt-0.5">Client: &ldquo;{item.clientNote}&rdquo;</div>
                      )}
                      {item.action && (
                        <span className={`text-[0.5625rem] font-medium ${item.action === 'accepted' ? 'text-success' : 'text-danger'}`}>
